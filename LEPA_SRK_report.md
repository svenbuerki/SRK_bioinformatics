...
**Synonymy group structure.** The 139 HV-identical allele pairs (d = 0) form nine tight synonymy groups within Class I, ranging from 2 to 14 alleles per group ([Figure 15d](#figure-15d)). These nine W-groups account for 41 of the 62 Class I alleles; the remaining 22 Class I alleles and the single Class II allele (Allele_061) are isolated — they share no HV-identical partner among the sampled bins. If HV identity reliably predicts shared recognition specificity, this would collapse the 63 sequence-defined bins to **31 functional groups**: 9 W-groups plus 22 isolated alleles (full group assignments in `SRK_synonymy_groups.csv`). The largest W-group (W-group 1) comprises 14 alleles observed in a combined total of 230 individuals, making it the most prevalent specificity in the dataset and therefore the highest priority for synonymy confirmation by crossing.

The connectivity structure among groups through N-connections (small but non-zero HV differences, 0 < d < 0.04) is visualised in a condensed super-node graph ([Figure 15e](#figure-15e)), where each node represents a W-group or isolated allele, and edges indicate the presence of at least one N-cross candidate bridging the two groups. Node size is proportional to the number of individuals observed carrying any member allele. The graph reveals that most W-groups and isolated alleles are reachable from one another through chains of N-edges, collapsing into three large N-connected components (56, 4, and 2 alleles; 1 isolated). This chaining reflects the continuous nature of HV variation within Class I and confirms that N-crosses will provide extensive coverage of the allele network — but also underlines that W-group membership (HV identity, d = 0) is the conservative and biologically robust criterion for synonymy prediction, while N-edges represent hypotheses that require experimental confirmation.

<a name="figure-15d"></a>

![Figure 15d: Allele synonymy network — HV-identical W-groups. Left panel: the nine synonymy groups (coloured nodes connected by red W-edges) and 22 isolated alleles (grey nodes, right panel). Node size is proportional to the number of individuals observed carrying each allele. Labels show allele ID and observation count. Together the nine W-groups contain 41 of the 63 alleles; if confirmed synonymous by crossing, these reduce to 9 functional groups, giving 31 total functional specificities from 63 sequence bins.](figures/SRK_synonymy_network_W.png)

...
- **[`tables/Phase5/step22b_synonymy_groups.csv`](tables/Phase5/step22b_synonymy_groups.csv)** — allele → synonymy group mapping (8 synonymy groups + 19 isolated → 27 effective bins) for collapsing putatively identical alleles in functional analyses (Step 22b).
...
![Figure 6a: Per-individual SI system status at the species level — robust subset (n = 258: SI + pSI + SC; drops Insufficient_data). Bars show SI / pSI · high / pSI · low / SC. **The species is dominated by SI** (247 / 95.7 % of the robust subset); pSI is 15 / 5.8 %; SC is 1 / 0.4 %. The companion side panel restates the same counts with category labels. The single SC individual is `Library010_barcode53` in EO76 / BL3. Source: `figures/Phase4/step25b_SI_status_species_robust.png` produced by `SRK_SI_status_figures.R`. A full-dataset variant (`SRK_SI_status_species_full.png`) shows the Insufficient_data and chimera tiers for QC use.](figures/Phase4/step25b_SI_status_species_robust.png)

<a name="figure-6b"></a>

![Figure 6b: SI status by Bottleneck Lineage — robust subset. Stacked bars are BL4 (n = 77, SI 72 / pSI 5), BL5 (n = 73, SI 70 / pSI 3), BL3 (n = 47, SI 44 / pSI 2 / SC 1), BL1 (n = 6, SI 5 / pSI 1), BL2 (n = 49, SI 45 / pSI 4). Every BL is dominated by SI; pSI is rare and roughly uniform across BLs. **BL3 carries the only confirmed SC individual** (in EO76). Source: `figures/Phase4/step25b_SI_status_by_BL_robust.png`.](figures/Phase4/step25b_SI_status_by_BL_robust.png)

<a name="figure-6c"></a>

![Figure 6c: SI status by focal Element Occurrence — robust subset, faceted by parent BL. EO27 (BL4) carries the most pSI carriers (4); EO70 (BL2) also carries 4; EO76 (BL3) carries the only confirmed SC individual + 2 pSI. **The pSI distribution is sparse and geographically diffuse**, not clustered in any one BL or EO. EO76 is distinguished by the SC presence and by the heavy Insufficient_data load in the full-dataset variant. Source: `figures/Phase4/step25b_SI_status_by_EO_robust.png`.](figures/Phase4/step25b_SI_status_by_EO_robust.png)

<a name="figure-6d"></a>

![Figure 6d: Projected per-copy broken-allele frequency (p_NULL) trajectories per BL under four scenarios. Thin lines = individual replicate trajectories (n = 30 reps per scenario per BL); bold lines = median trajectory across replicates. Initial state = empirical Step 26 per-BL genotypes. **Larger BLs (BL3, BL4, BL5) drift slowly upward but most replicates stay below p_NULL = 0.5 within 100 generations.** BL1 (n = 7) shows the widest replicate spread — drift dominates at small N. Source: `figures/Phase4/step27_inheritance_pNULL_trajectories.png`.](figures/Phase4/step27_inheritance_pNULL_trajectories.png)

<a name="figure-6e"></a>

![Figure 6e: Projected SC frequency over time per BL. Bold lines = median across 30 replicates; shaded bands = inter-quartile range. Dashed horizontal line = 50 % SC tipping-point threshold. **BL2 reaches 50 % SC fastest under baseline (~26 generations);** BL3 (~41), BL5 (~62), BL4 (~66) take longer; BL1 hovers near the threshold without reliably crossing it. Source: `figures/Phase4/step27_inheritance_SC_progression.png`.](figures/Phase4/step27_inheritance_SC_progression.png)

<a name="figure-6f"></a>

![Figure 6f: Median time to 50 % SC frequency per BL × scenario. Bars = median across replicates that reached the threshold; whiskers = IQR; labels above bars = number of replicates reaching SC / total replicates per group. Coloured by scenario. The visual reading: baseline timelines range from ~26 generations (BL2) to > 100 (BL1, BL5 under several rescue scenarios). Source: `figures/Phase4/step27_inheritance_time_to_sc.png`.](figures/Phase4/step27_inheritance_time_to_sc.png)

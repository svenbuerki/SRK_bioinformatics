...
|-------|-------|--------------|------------------|
| **1. [Amplicon assembly + phasing](Bioinformatics_pipeline.md#phase-1-srk-amplicon-sequence-assembly)** | 1–8 | Multi-replicate Canu assembly, RACON polishing, polyploid-aware variant calling, WhatsHap read-backed phasing, MAFFT alignment, exon-projection, gap backfilling, frame-correct translation. Two targeted QC filters (BLAST coverage + N-content) address Library-010 failure modes. | Per-individual phased SRK haplotypes |
| **2. [Allele definition + genotyping](Bioinformatics_pipeline.md#phase-2-functional-proteins-s-alleles-and-genotyping)** | 9–12c | Abundance-filter functional proteins, distance-based S-allele clustering (Kneedle-calibrated `N_ALLELES`), per-individual allele copy-count genotyping, tetraploid zygosity (AAAA / AAAB / AABB / AABC / ABCD), and a Phase-2 data-quality gate that splits samples into Functional / Re-PCR / Re-DNA-extraction / SI-escape categories. | Functional proteins, allele bins, individual genotypes, lab follow-up CSV |
| **3. [Population genetics + conservation diagnostics](Bioinformatics_pipeline.md#phase-3-data-analyses)** | 13–21 | Bottleneck-Lineage integration via [`LEPA_EO_spatial_clustering`](https://github.com/svenbuerki/LEPA_EO_spatial_clustering); allele accumulation + Michaelis-Menten / Chao1 / iNEXT richness estimation; mating-pool viability (`P_compat`) + Depletion Index ranking with bootstrap CIs; UpSet + Euler set diagrams; Genotypic Fitness Score; TP2 tipping-point + reproductive-effort support. | Lineage stratification, mating + fitness metrics, lab re-sequencing CSV |
| **4. [Per-individual SI status + forward simulation](Bioinformatics_pipeline.md#phase-4-per-individual-si-status-null-aware-genotypes-and-forward-simulation)** | 22–25 | AA-distance broken-allele identity (chimera filter), per-individual SI / pSI / SC classification, null-aware tetraploid genotype rebuild + downstream Phase-3 re-runs (14b/17b/19b/20b), forward-time inheritance simulator (4 scenarios × 5 BLs), donor ranking for allele injection. | SI status per individual, null-aware metrics, donor ranking |

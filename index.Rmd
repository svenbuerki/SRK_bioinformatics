...
This pipeline reconstructs S-receptor kinase (SRK) haplotypes from Nanopore amplicon sequencing of polyploid plants and turns them into population-genetic and crossing-design data products usable for conservation breeding. It comprises **28 steps in 5 phases**.

## Roadmap — what the pipeline does, at a glance

Read the figure below left-to-right: each phase shows what it does (bullets), the steps it covers, and the canonical output it produces. Phase 2 → 3 → 4 → 5 each consume the previous phase's outputs.

<figure style="margin: 1.5em 0;">
<img src="figures/SRK_pipeline_overview.png"
     alt="SRK pipeline overview — 5-phase roadmap"
     style="width: 100%; max-width: 1800px; display: block; margin: 0 auto;">
<figcaption style="font-style: italic; text-align: center; margin-top: 0.5em; font-size: 0.9em;">
SRK pipeline overview — 5-phase roadmap from Nanopore amplicons through functional S-alleles to conservation diagnostics, cross plan, and forward-time simulation. Produced by <code>SRK_pipeline_overview_figure.R</code>. (Click to enlarge in a new tab.)
</figcaption>
</figure>

For step-by-step detail (scripts, inputs, outputs, key parameters) see the protocol document [`Bioinformatics_pipeline.md`](https://github.com/svenbuerki/SRK_bioinformatics/blob/main/Bioinformatics_pipeline.md).

## Two design choices worth flagging up front

### Chimera defence in two stages

Canu *de novo* assembly of Nanopore amplicons routinely produces chimeric contigs — reads from two different SRK haplotypes get spliced into a single false haplotype, and the splice junction typically introduces a premature stop codon. Left in the data, chimeras look molecularly indistinguishable from real broken SRK copies and inflate the per-individual broken-copy count, producing false `pSI` / `SC` calls. Because chimeras leave a different fingerprint at each stage, the pipeline filters them twice:

1. **Step 2 — coverage-based filter on the assembled DNA contigs.** `chimera_coverage_filter.py` runs inside the within-library assembly wrapper, after `seqkit rmdup` and before RACON polishing. It computes per-contig `uniformity = min_window_cov / median_cov` across a 200 bp sliding window and **drops contigs with `mean_cov < 20` OR `uniformity < 0.20`**. Threshold calibrated on Library 005 (2026-06-12) — the highest DROP sits at `uniformity = 0.182` and the lowest KEEP at `0.202`, a clean gap. Chimeric contigs show characteristic mid-contig depth crashes (high plateau collapsing to ~10×) that this metric isolates. Configurable via `CHIMERA_FILTER_MODE` (`report` for calibration vs `filter` for production), `MIN_MEAN_COV`, `MIN_UNIFORMITY`, `CHIMERA_WINDOW`. Stats land in `logs/${library}_${run_id}_${barcode}.chimera_stats.tsv`.
2. **Step 22a — AA-distance filter on translated haplotypes.** `SRK_null_allele_assignment.py` aligns every haplotype tagged `REMOVED` by Step 7 (premature stop) to the 49 canonical functional allele representatives via `mafft --add --keeplength` and computes amino-acid p-distance. Confidence tiers: `high` (`AA_distance ≤ 0.005` AND `n_stops ≤ 3`) and `medium` (`AA_distance ≤ 0.05` AND `n_stops ≤ 10`) are **real broken copies** of a known allele — they enter the Step 22b SI status calculation. `low` confidence (`AA_distance > 0.05` OR `n_stops > 10`) are chimeric Canu artefacts that resemble no known allele — they are recorded as `n_haps_chimeric` for data-quality transparency but excluded from the tetraploid-copy signal pool used by Step 22b. Also records `first_stop_position` and `stop_codon_DNA` for downstream IBD work distinguishing same-mutation (IBD) from independent-LoF (IBS) signals on the same allele.

Outputs: `Tables/Phase4/step22a_null_allele_assignments.tsv` (per-haplotype identity + confidence) consumed by Step 22b. Only individuals whose surviving haplotypes pass both layers reach a defensible SI / pSI / SC call.

### Lab re-run identification — a single merged sheet

`evaluate_data_quality.py` (Step 12c) is the only place in the pipeline where samples are flagged for lab follow-up. It reads the per-sample exit point from every upstream stage — Phase 1 length / paralog / N-content / no-assembly failures + Phase 2 abundance / contamination failures — AND joins the Phase 4 Step 22b `Insufficient_data` individuals. The merged CSV at **`tables/Phase2/step12c_samples_redo.csv`** carries one `Lab_action` per row:

- **Re-PCR** — DNA stock fine; the PCR amplicon failed. Triggered by Stage 4b BLAST paralogs, Step 9 dropped_abundance_filter, Step 9 no_functional_proteins, Stage 3 no assembly, or Step 4 length-filter failures. Same-day operation; re-amplify from existing DNA.
- **Re-DNA-extraction** — DNA stock itself is contaminated (Step 9 `too_many_alleles` — individuals showing 15–26 distinct functional proteins). Requires fresh tissue isolation; re-PCR would preserve the contamination.
- **Re-sequence (SI status uncertain)** — added 2026-06-15 augmentation. Step 22b classified the individual as `Insufficient_data` (`n_haps_OK + n_haps_REMOVED_real < 4` after the Step 22a chimera filter, so the tetraploid copy budget is undefined). Existing DNA and library are fine; higher-coverage Nanopore re-run on the sequencer will resolve the call. The `Redo_reason_SI` column carries the per-sample diagnostic `Insufficient_data (n_haps_OK=…, REMOVED=…, chimeric=…)`.

The CSV's full schema is `Lab_action, Sample_ID, Tube_number, Library, Barcode, EO, EO_normalised, BL_inferred, n_raw_haps, Proteins_in_final_data, Exclusion_stage, Recommended_action, Redo_reason_SI` and rows are sorted by `Lab_action → EO → Sample_ID` for field-collection grouping. This is the single file collaborators consult — root-level `Tables/SRK_samples_for_redo.tsv` and `Tables/SRK_samples_redo.csv` are pre-2026-06-14 paths and were removed from the GitHub repo in commit `8f8e820`.
...
**Phase 2 — Allele definition and genotyping** (Steps 10–12). Distance-based clustering of validated proteins into S-allele bins ([Step 10](#SRKalleledist)), per-individual allele copy-count genotyping ([Step 11](#SRKfunctionalprotgenotype)), and tetraploid zygosity classification into `AAAA`/`AAAB`/`AABB`/`AABC`/`ABCD` genotypes ([Step 12](#zygosity)). The current dataset comprises **335 ingroup individuals across 19 *Lepidium papilliferum* Element Occurrences resolved into 49 observed S-allele bins** (Kneedle calibration N = 58 defined bins; 9 unobserved bins are reference-allele placeholders).

**Phase 3 — Population genetics under a bottleneck-lineage framework** (Steps 13–21). [Step 13](#BLintegration) imports five independent bottleneck lineages (BL1–BL5) from the sibling repository [LEPA_EO_spatial_clustering](https://github.com/svenbuerki/LEPA_EO_spatial_clustering) — derived by Ward's D2 hierarchical clustering of 32 geographic-group centroids under a 500 m pollinator-connectivity threshold — and writes a per-individual BL assignment table consumed by every downstream analysis. Steps 14–21 then compute population-genetic statistics, allele-accumulation curves with Michaelis-Menten and Chao1 estimators, χ² goodness-of-fit against the NFDS equal-frequency expectation, the TP1 mating-pool functionality diagnostic (evenness J × tetraploid sporophytic-SI compatibility P_compat, at strict and leaky SI), allele composition / sharing (UpSet + heatmap), per-individual Genotypic Fitness Score, the TP2 tipping-point diagnostic (mean GFS × proportion AAAA), and reproductive-effort summaries — every output stratified in parallel by EO (management unit, ordered within parent BL by ascending mean Drift_index) and by BL (inferential unit, ordered by total habitat area then within-BL connectivity: **BL4, BL5, BL3, BL1, BL2**). The locked Set1 colour palette (BL1 purple, BL2 blue, BL3 red, BL4 orange, BL5 green) is used consistently with the sibling spatial-clustering repository. All BL ordering and palette decisions are centralised in the mirrored modules **`srk_bl_constants.R`** and **`srk_bl_constants.py`**, sourced/imported by every Phase 3/4 script (see Step 14 for the full ordering rules).

**Phase 5 — Hypothesis testing and crossing design** (Steps 26–27). [Step 26a](#alleleclusters) (`srk_variability_landscape.py`) runs an identical sliding-window Shannon-entropy scan on a combined alignment of LEPA + 22 *Brassica* + 10 *Arabidopsis* SRK alleles to detect hypervariable (HV) columns per species, validates LEPA HV regions against Brassica/Arabidopsis with a 10 000-permutation overlap test, and overlays the 12 SCR9-contact residues from Ma et al. 2016 (PDB 5GYY) for structural validation. Step 26b (`srk_allele_hypotheses.py`) consumes the canonical HV columns to compute HV-only pairwise distances, UPGMA-detect Class I / Class II, build the synonymy network (synonymy groups (HV-identical) + Synonymy_test bridges), and rank the AAAA × AAAA cross matrix into Incompatible / Synonymy_test / Compatible_within / Compatible_cross categories. Two optional diagnostics — **Step 26c** (`srk_wgroup_collapse_test.py`, tests how much Synonymy group redundancy contributes to LEPA's low Shannon entropy) and **Step 26d** (`srk_perBL_entropy_test.py`, decomposes LEPA HV-column residue diversity per Bottleneck Lineage and compares to Brassica + Arabidopsis to distinguish drift from selection convergence) — flesh out the biological mechanism. **Step 26e** (`srk_cross_plan.py`) translates the cross categorisation into a phased experimental protocol (H0 / H1a / H1b / H2 / H3) with explicit AAAA / AAAB / AABB genotype requirements and paired controls for heterozygous-donor tests; every cross row is traceable to a sequence-based category (HV distance + Class from Step 26b) plus a genotype-based feasibility filter (`Allele_composition` from Step 12). [Step 27](#crossresults) reads completed cross records and tests the predictions.
...
## Output organisation (2026-06-14 refactor — all 5 phases)

Every pipeline script reads/writes from a Phase- and step-prefixed path:

- **Tables**: `Tables/Phase{2,3,4,5}/stepXX_*.{tsv,csv,fasta}`
- **Figures**: `figures/Phase{2,3,4,5}/stepXX_*.{pdf,png}` — every plot now produces **both PDF and PNG** (vector + raster) except inherently multi-page PDFs (Step 15 curves, Step 16 frequency, Step 19 GFS, Step 16 chisq) where the page-level PNGs are companions.
- **External reference FASTAs** (Brassica, Arabidopsis SRK references for Step 26) live in `FASTA/`.
- `Tables/sampling_metadata.csv` is the single canonical metadata file (symlink at `./sampling_metadata.csv` for legacy scripts).

**Phase 5 (Step 26) update:** `pad_representatives.py` now restricts LEPA representatives to ingroup-observed alleles only (49 of 55), removing outgroup contamination from the cross-Brassicaceae variability landscape analysis.

**Phase 4 (Steps 22–25 + null-aware 14b/17b/19b/20b reruns)** writes to `Tables/Phase4/` and `figures/Phase4/` with `step22a/22b/23/14b/17b/19b/20b/24/25_*` prefixes.

See `Bioinformatics_pipeline.md` for the full directory tree.
...
**Step 12c — [Data Quality Evaluation](#sampleaudit)** (Phase 3 QC gate). A formal data-quality gate that asks three sequential questions: (i) is there a library effect impacting interpretation? (`test_library_effect.py`); (ii) which samples need lab follow-up? (`audit_sample_exclusions.py` + `evaluate_data_quality.py` → **`Tables/SRK_samples_redo.csv`** with a `Lab_action` column distinguishing Re-PCR (existing DNA OK, re-amplify) from Re-DNA-extraction (DNA contaminated, re-isolate tissue)); (iii) which samples have non-functional SRK proteins and may have escaped SI? (→ `Tables/SRK_SI_escape_candidates.csv`). Visualises outcome-category proportions per BL and per focus EO so populations with elevated lab-follow-up or SI-escape rates are obvious at a glance. **Execution order (revised 2026-06-13):** 12c.ii + 12c.iii run after Step 13 (BL integration) so per-BL stratification is available; 12c.i (library effect) runs after Step 19 (GFS) since it consumes `SRK_individual_GFS.tsv`.
...
13b. [Sampling Overview Map](#samplingmap) – Single-panel geographic map plotting every catalogued population against its parent BL. Filled triangles = sampled by the SRK pipeline; filled circles = not yet sampled. Recommended first visual in any presentation or report.
...
17. [TP1 — Mating-pool functionality](#TP1) – **Is the depleted population still functioning?** Per-EO and per-BL evenness J × tetraploid sporophytic-SI compatibility P_compat, at strict (L = 0) and empirical-leakage (L = 0.25) levels. Quadrants map to conservation actions (MONITOR / AUGMENT-evenness / AUGMENT URGENTLY / BIOBANK + RESTORE). Connects directly to Stage 5 of the C3 cascade.
...
## [Phase 4: Per-individual SI status, null-aware genotypes, and forward simulation](#phase4)

> Phase 4 establishes the per-individual self-incompatibility status. This information is *required* before Phase 5 (cross design) — a cross plan is only meaningful when the parents have a verified functioning SI machinery.

22. **Per-individual SI / pSI / SC reconstruction** – `SRK_individual_SI_status.py` re-aggregates per-haplotype OK / REMOVED calls to classify each individual as full SI, partial SI, self-compatible, or insufficient_data. `SRK_null_allele_assignment.py` augments this by mapping REMOVED haplotypes to their nearest functional allele via amino acid distance.
23. **Null-aware genotype rebuild** – `SRK_genotype_null_alleles.py` rewrites the allele × individual matrix so that partial SI individuals carry explicit `Allele_NULL` copies and self-compatible individuals are represented as `Allele_NULL × 4`. Triggers null-aware cascade re-runs of Steps 14b (population genetics), 17b (random-mating viability), and 19b + 20b (fitness scores + reproductive collapse risk).
24. **Forward-time tetraploid inheritance simulator** – `SRK_inheritance_simulator.py` simulates per-lineage trajectories under multiple scenarios (baseline / high_drift / rescue_high / rescue_low) starting from the empirical Step 23 genotypes.
25. **Donor ranking for allele injection** – `SRK_injection_donor_ranking.py` ranks candidate seed parents for allele-injection crosses to rescue specific lineages / populations.

## [Phase 5: Testing S-allele Hypotheses and Cross Design](#phase5)

> Phase 5 uses the per-individual SI status established in Phase 4. Cross design is restricted to parents whose Phase 4 SI status is `SI` — broken-SI individuals cannot serve as reliable compatibility-prediction parents.

26. [Hypervariable-Position Allele Hypothesis Testing and Crossing Design](#alleleclusters) – Cross-Brassicaceae sliding-window Shannon-entropy scan identifies 66 canonical HV columns (anchored structurally by 11 of the 12 mappable Ma 2016 SCR9-contact residues; cross-genus permutation overlap with Brassica + Arabidopsis is no longer significant in the current post-QC dataset — interpreted as drift-eroded standing variation, see Step 26d); UPGMA auto-threshold clustering detects the Class I / Class II phylogenetic split (Class II = single allele, Allele_055); generates a sequence-based cross design for AAAA × AAAA pairs with four categories (Incompatible, Synonymy_test, Compatible_within, Compatible_cross) — restricted to Phase-4-verified SI parents. Optional diagnostics 26c (Synonymy group collapse test) and 26d (per-BL entropy decomposition) test alternative mechanisms underlying LEPA's low entropy. **Step 26e** ([Hypothesis-Testing Cross Plan](#crossplan)) translates the sequence-based categories into a phased experimental protocol with explicit AAAA / AAAB / AABB genotype constraints (H0 SI validation, H1a within-Class baseline, H1b between-Class baseline, H2 synonymy bin boundaries, H3 hidden bins via heterozygous donors + paired controls).
27. [Cross Result Analysis](#crossresults) – Statistical comparison of seed yield across Incompatible / Synonymy_test / Compatible_within / Compatible_cross cross categories to test whether HV-based allele bin boundaries predict functional recognition specificity.
...
- LibraryID MUST be e.g. `Library001`. Avoid underscore to support downstream analyses.
- **Recommended end-to-end driver:** `./run_within_library_suite.sh` chains Steps 2 → 8 for every library in `libraries.tsv` (auto-generated from `Library*/barcode??`), pipes each step's prompts automatically, skips already-completed samples / steps, and continues past per-library failures. See `Bioinformatics_pipeline.md` for env vars (`CHIMERA_FILTER_MODE`, `FORCE`, `FORCE_SAMPLE`, `FORCE_CANU`, `START_AT_STEP`, `STOP_AT_STEP`) and resume semantics.
- **Step 2 (this script) when run individually:**  `./nanopore_assembly_pipeline_barcode_range.sh` and enter info when prompted (LibraryID and range of barcodes). Failure-tolerant (per-sample isolation), with an embedded coverage-based chimera filter (`chimera_coverage_filter.py`) between Canu rep merging and RACON polishing, Canu cache resume, and per-sample skip when `${Library}_${barcode}_Phased_haplotypes.fasta` already exists.
...
## Step 12c — Data Quality Evaluation (Phase 3 QC gate) {#sampleaudit}

### Overview

A formal data-quality gate that runs as part of Phase 3 — specifically after Step 13 (BL integration) for the audit + categorisation sub-scripts (12c.ii, 12c.iii) and after Step 19 (GFS) for the library-effect tests (12c.i). It asks **three sequential questions** and produces deliverables for both the bioinformatics record and the wet-lab team.

1. **Q1 — Is there a library effect impacting data interpretation?** (`test_library_effect.py`)
2. **Q2 — Which samples failed sequencing/assembly and need to be re-sequenced?** (`audit_sample_exclusions.py` + `evaluate_data_quality.py` → `SRK_resequencing_priorities.csv`)
3. **Q3 — Which samples have non-functional SRK proteins (full to partial) and may have escaped self-incompatibility?** (→ `SRK_SI_escape_candidates.csv`)

Three sub-steps execute in sequence:

| Sub-step | Script | Question | Output |
|---|---|---|---|
| 12c.i  | `test_library_effect.py`        | Q1 — Library effect tests       | `SRK_library_effect_tests.tsv`, `SRK_library_effect_summary.pdf` |
| 12c.ii | `audit_sample_exclusions.py`    | Q2 + Q3 — Per-sample audit      | `SRK_sample_exclusion_audit.tsv` |
| 12c.iii | `evaluate_data_quality.py`     | Integrate + CSVs + plots        | `Tables/SRK_data_quality_categories.tsv`, `Tables/SRK_samples_redo.csv`, `Tables/SRK_SI_escape_candidates.csv`, `figures/SRK_data_quality_per_{BL,EO}.png` |

Even fully non-functional samples (Complete_loss; excluded from the final genotype table) are *flagged* in the categorisation rather than silently dropped — they carry biologically informative signal about ongoing SI escape.

### Q1 — Library effect testing (Step 12c.i)

`test_library_effect.py` runs four tests:

| Test | Question | Method |
|---|---|---|
| 1 | Does `SI_functional_status` depend on library globally? | χ² of Library × {Functional, Partial_translation_failure, Complete_loss} |
| 2 | Does `Dominant_failure_mode` depend on library globally? | χ² of Library × {premature_stop, ambiguous_aa, mixed, other} |
| 3 | Does AAAA proportion depend on library *within an EO*? | Fisher's exact per EO between Library 009/010 and pooled "other" |
| 4 | Does Complete_loss proportion depend on library *within an EO*? | Fisher's exact per EO between Library 009/010 and pooled "other" |

**Interpretation rule:** a significant Test 2 (failure-mode association) is *expected* if libraries differ in pipeline processing (e.g., Library 010 should be enriched in `premature_stop` because Step 7b removes the `ambiguous_aa` noise upstream). This is a *favourable* technical signal, not a confound. A significant Test 1 (SI-status association) would be a confound; Tests 3 + 4 are the cleanest test for within-EO bias.

### Q2 + Q3 — Audit + categorisation (Steps 12c.ii + 12c.iii)

`audit_sample_exclusions.py` first produces the per-sample audit (see the cascade in its own subsection below for full detail). `evaluate_data_quality.py` then reads the audit + library-effect outputs and assigns every metadata sample to one of **five mutually exclusive outcome categories**:

| Category | Criterion | Lab action |
|---|---|---|
| **Functional** | `SI_functional_status = Functional` (included) | None — feeds Phase 3 |
| **Partial_translation_failure** | `SI_functional_status = Partial_translation_failure` (included) | None — included. **NEUTRAL label: this is most likely a technical artefact (chimeric Canu haplotypes inflating the failure denominator), not biological SI loss. The current dataset provides no tangible evidence supporting biological SI loss in this category — make no inference about SI status from this label alone.** |
| **SI_escape_candidate** | `SI_functional_status = Complete_loss` AND `Dominant_failure_mode = premature_stop` (excluded) | Phenotype via controlled selfing test |
| **Re_PCR** | Stage 3 / Step 4 length / Step 4b BLAST paralog / Step 7b / dropped_abundance_filter / no_functional_proteins with ambiguous_aa or mixed failures | **Re-amplify SRK from the existing DNA stock.** The DNA is fine; the PCR product is the problem (no assembly, fragmented amplicon, paralog amplification, short product, low yield, dirty product translating to X residues) |
| **Re_DNA_extraction** | Step 9 too_many_alleles (>4 distinct functional proteins per individual) | **Re-isolate tissue from a single plant + re-extract DNA before any further PCR.** The DNA stock itself is contaminated; re-PCR will preserve the contamination |

(Outgroup samples — `Ingroup = 0` in metadata — are flagged separately and not shown in the BL/EO plots.)

The Re_PCR / Re_DNA_extraction split is operationally meaningful: re-PCR is a same-day same-DNA operation, while re-DNA-extraction requires fresh tissue (possibly field collection) before any PCR can happen. Surfacing them as separate CSV deliverables (`SRK_re_PCR_priorities.csv`, `SRK_re_DNA_extraction_priorities.csv`) lets the wet-lab team triage and schedule appropriately.

### Two distinct questions answered by one audit

1. **QC question:** which samples need re-sequencing vs which have an intrinsic data-integrity problem (paralogs, contamination) where re-sequencing will not help?
2. **Biology question:** which samples have successful sequencing but produced *no functional SRK proteins*? Such samples carry zero rejection capacity at the molecular level and are candidates for ongoing self-compatibility (SC) evolution via loss-of-function (LoF) mutations.

### Exclusion cascade (earliest stage wins)

| Stage | Reason | Re-sequencing? |
|---|---|---|
| **Stage 3 — no assembly** | sample in metadata, Canu produced 0 haplotypes | ✓ yes |
| **Step 4 — length filter** | raw haps exist, 0 passed 3250–4000 bp | ✓ yes |
| **Step 4b — BLAST coverage** | all haps dropped at 0.90/95 % (Library 010+) | ✗ no — SRK paralogs / off-targets |
| **Step 7b — N content** | all haps dropped at max_N = 9 (Library 010+) | maybe — short Canu contigs |
| **Step 9 — no_functional_proteins** | no protein passed translation/validation | depends on `Dominant_failure_mode` (see below) |
| **Step 9 — dropped_abundance_filter** | functional proteins below `min_count = 5` | maybe — singletons may pass with more reads |
| **Step 9 — too_many_alleles** | >4 distinct proteins per individual | ✗ no — likely contamination / mixed sample |
| **Step 11 — outgroup filter** | `Ingroup = 0` in metadata | intentional exclusion |

The `BL_status = Unassigned` flag (10 individuals with germplasm sub-codes) is **not** an exclusion — these samples are in the zygosity matrix; they just cannot be placed in a BL for Phase 3 stratification.

### Biological columns — `SI_functional_status` + `Dominant_failure_mode`

For samples that reached Step 9 translation, the audit categorises the SI functional state:

| `SI_functional_status` | Criterion | Interpretation |
|---|---|---|
| Functional | `Functional_rate ≥ 0.5` | SI system intact at the molecular level |
| Partial_translation_failure | `0 < Functional_rate < 0.5` (low_functional_rate) | included in dataset. **NEUTRAL — does NOT imply biological SI loss.** Most likely a technical artefact from chimeric Canu haplotypes (spliced junctions introduce premature stops that inflate the failure denominator). Current data provides no tangible evidence supporting biological SI loss in this category. Make no inference about SI status. |
| **Complete_loss** | `Total_sequences > 0` AND `Functional_proteins = 0` | sequencing successful but no functional protein recovered — **candidate SI-escape signal** |
| NA | did not reach Step 9 | excluded earlier in pipeline |

`Dominant_failure_mode` discriminates biological vs technical signal at the Complete_loss tier:

| Mode | Meaning | Biological signal |
|---|---|---|
| `premature_stop` | internal stop codons dominate (≥ 2× the second most common failure) | **real loss-of-function mutations — strong SI breakdown candidate** |
| `ambiguous_aa` | X residues from N-rich data dominate | **data quality artefact, NOT biological SI escape** (typical of libraries that bypassed Step 7b) |
| `mixed` | second-most-common ≥ 50 % of top | interpret cautiously |
| `other` | different pattern | rare; investigate manually |

The combination `Complete_loss` + `premature_stop` is the strongest molecular candidate for ongoing SC evolution and warrants follow-up phenotyping (controlled selfing tests).

### When to run

After Step 12 completes, every time a new library is added. The audit takes < 1 s and is library-agnostic — it auto-detects which per-library logs (Step 4b, Step 7b, Step 8) are available and adapts accordingly.

### Bioinformatics

- Script: `audit_sample_exclusions.py` (root of `Canu_amplicon/`)
- Inputs (auto-detected in CWD):
    - `sampling_metadata.csv` — ground truth list of expected samples
    - `all_Library*_Phased_haplotypes.fasta` — per-library Step 3 raw haplotypes
    - `all_Library*_*_min3250*.fasta` — Step 4 length-filtered outputs
    - `all_Library*_*_blastfilt_log.tsv` — Step 4b decisions (Library 010+)
    - `all_Library*_*_backfilled_Nfilt_log.tsv` — Step 7b decisions (Library 010+)
    - `SRK_individual_status_report.tsv` — Step 9 classifications
    - `SRK_individual_BL_assignments.tsv` — Step 13 BL status (run after the audit on first iteration; or re-run audit after Step 13 to fill the BL_status column)
    - `SRK_individual_zygosity.tsv` — final included individuals
- Output: `SRK_sample_exclusion_audit.tsv` — one row per metadata sample (16 columns covering metadata, per-stage hap counts, Step 9 classification + `SI_functional_status` + `Dominant_failure_mode`, BL_status, final status + exclusion stage + reason).
- Stdout summary: cascade totals + SI-status breakdown + explicit list of candidate SI-escape samples for immediate triage.

### Headline finding (LEPA dataset, 2026-05-12)

Of **378 individuals** that reached Step 9 translation, **199 (53 %) are Functional**, **165 (44 %) are Partial_translation_failure**, and **14 (4 %) are Complete_loss**. The 165 Partial_translation_failure individuals are most likely capturing a technical artefact (chimeric Canu haplotypes inflating the failure denominator) rather than biological SI loss; **no biological inference is made from this category**. The biologically defensible signal comes from the 14 Complete_loss individuals: **7 are `premature_stop`-dominated candidate SI-escape samples**, requiring *every* observed haplotype to carry a stop, which is hard to explain by chimeras alone. Remarkably, **5 of these 7 cluster in EO76 (BL3)** — the same population that holds 0 AABC seed parents and the highest AAAA fraction. This molecular signal independently corroborates the demographic/genotypic picture and identifies EO76 as the population most likely undergoing active self-compatibility evolution. Follow-up controlled selfing tests are recommended to validate the molecular signal at the phenotype level. Full discussion in `LEPA_SRK_report.md` Q4.
...
Genetic drift reduces allele richness and skews remaining frequencies away from the equal-frequency NFDS expectation. Two complementary diagnostic levels are evaluated. **Steps 15–16** quantify *depletion* — how many alleles have been lost relative to the species optimum (≈ 58 alleles), and how their surviving frequencies deviate from the NFDS equal-frequency expectation. **Step 17 (TP1)** then takes that depletion as given and asks the complementary question: *is the depleted population still demographically functioning, and what conservation intervention does the data support?* TP1 places each EO and BL on a scatter of evenness J × tetraploid sporophytic-SI mating compatibility P_compat, with quadrants mapped directly to conservation actions (MONITOR / AUGMENT-evenness / AUGMENT URGENTLY / BIOBANK + RESTORE). Allele composition comparison across EOs ([Step 18](#allelesharing)) identifies which populations are reservoirs of unique allelic diversity.

Note that "genetic drift" here encompasses two conceptually distinct processes: (1) a discrete historical **bottleneck** — a sudden reduction in population size that causes rapid simultaneous loss of allele richness; and (2) **ongoing drift** in small populations — the continuous stochastic erosion of allele frequency evenness as dominant alleles accumulate. Steps 15–16 carry the richness signal (bottleneck footprint); TP1's evenness J axis carries the ongoing-drift signal. Populations scoring poorly on both — as observed across the focal LEPA EOs — are most consistent with a historical bottleneck followed by prolonged ongoing drift. The SRK locus is under balancing selection (NFDS) and cannot be used directly with standard neutral-marker bottleneck tests; formally partitioning the two processes requires genome-wide neutral marker data and demographic modelling (e.g., SMC++, PSMC).
...
## Sampling Overview Map {#samplingmap}

### Purpose

A single-panel geographic map showing every catalogued population of the species against its parent BL, with a clear visual distinction between populations successfully sampled by the SRK pipeline and not-yet-sampled populations. This is the **recommended first visual** in any presentation or report: it sets the geographic baseline before any per-locus analysis and makes the gap between current sampling and the full species range immediately obvious. Produced by `SRK_sampling_map.R` and built on top of the canonical post-QC allele matrix + Step 13 BL assignments.

### Inputs

| File | From step | Description |
|------|-----------|-------------|
| `Tables/SRK_individual_allele_genotypes.tsv` | [Step 11](#SRKfunctionalprotgenotype) | Canonical successful-sample roster (335 ingroup individuals) |
| `Tables/sampling_metadata.csv` | (registry) | `Pop` and `EO_w_sub` lookup per `SampleID` |
| `SRK_individual_BL_assignments.tsv` | [Step 13](#BLintegration) | Per-sample EO → BL resolution |
| `EO_location_groups.csv` | sibling [`LEPA_EO_spatial_clustering`](https://github.com/svenbuerki/LEPA_EO_spatial_clustering) | Lat/lon for every catalogued population |
| `Tables/EO_group_BL_summary.csv` | sibling project (mirror) | Parent BL for every catalogued EO (so unsampled populations still get their BL colour) |
| `srk_bl_constants.R` | (constants) | Locked Set1 BL palette + `BL_ORDER` |

### Visual conventions

- **Shape**: filled triangle = sampled; filled circle = not yet sampled. Both filled with the parent BL colour from the locked Set1 palette.
- **Size**: habitat area (ha), `sqrt`-scaled so small populations stay visible.
- **Labels**: for each sampled EO, the EO code (with `n =` sample count) is placed on the *largest* location within that EO — this is the convention for compound germplasm sub-codes that cannot be pinned to a specific sub-location. Focus EOs (EO18, EO25, EO27, EO67, EO70, EO76) carry bold white-fill labels; other sampled EOs carry smaller plain labels. Unsampled populations are left unlabelled to keep the map readable.
...
| File | Content |
|------|---------|
| `figures/SRK_sampling_map.png` / `SRK_sampling_map.pdf` | Captioned version |
...
### Headline numbers (current dataset, 2026-05-21)

**325 BL-resolved samples across 17 populations in 16 EOs**, with the following per-BL distribution: BL1 (5 / 4 / 4) · BL2 (59 / 2 / 2) · BL3 (80 / 4 / 4) · BL4 (89 / 2 / 2) · BL5 (92 / 4 / 5). Plus **10 samples / 9 germplasm sub-codes in the JAR region** (no coordinates in the spatial catalogue; not plotted on the map). Sum = **335 total successful ingroup samples**.
...
Two-page PDF, four panels per page (Effective allele diversity, Heterozygosity, Allele richness, Sample size). **Page 1**: EO bars in area-then-connectivity order (parent BL by `BL_ORDER` → EOs by ascending mean Drift_index), bars coloured by BL using the locked Set1 palette (legend in panel 1). **Page 2**: BL aggregate bars in area-then-connectivity order (BL4, BL5, BL3, BL1, BL2).

The Set1 BL palette and ordering rules established here are sourced by Steps 15–21 from the shared module `srk_bl_constants.R` (Python equivalent `srk_bl_constants.py`) and match the sibling LEPA_EO_spatial_clustering project:

| BL  | Hex       | Colour | Connectivity rank |
|-----|-----------|--------|------------------:|
| BL1 | `#984EA3` | purple |                 2 |
| BL2 | `#377EB8` | blue   |                 5 |
| BL3 | `#E41A1C` | red    |                 4 |
...
The connectivity rank (`n_locations − n_groups`, tie-broken by `total_pop_size`, both descending) drives `BL_ORDER` (used for bars, faceted panels, axes, tables). For scatter-plot legends (TP1, TP2, accumulation-curve legends) where the axes are not BL and a readable legend matters more than the inferential order, use `BL_ORDER_NUMERIC` (alphanumeric BL1→BL5). EOs within each BL are ordered by ascending mean Drift_index via `get_eo_order_within_bl()` — same formula in R and Python.
...
* Species-level accumulation curve (all 335 ingroup individuals)
* One page per EO with N ≥ 5 (ordered by parent BL connectivity → ascending mean Drift_index)
* One page per BL (all 5 BLs, regardless of N, in connectivity order BL5→BL2)

Each page also overlays MM, Chao1, and (optional) iNEXT asymptote reference lines.

**Standalone PNGs (in `figures/`)**

* `figures/SRK_allele_accumulation_species.png` — two-panel species figure: stacked bar (observed + predicted-undetected = MM) on the left, rarefaction curve with MM/Chao1/iNEXT asymptotes on the right, both sharing the bar's y-axis. The bar uses the same dark-blue/light-blue colour key as the BL/EO drift-erosion bars, so the species, BL, and EO panels can be read together.
* `figures/presentation/SRK_allele_accumulation_species_observed.png` / `_predicted.png` — slide build-up frames of the figure above: frame 1 shows only the bar's observed segment (no curve, no MM line); frame 2 shows the full bar + curve + MM dashed line (Chao1/iNEXT suppressed). Same 12 × 6.5 in canvas across frames for clean animation.
...
* `figures/SRK_allele_accumulation_species.png` — bar (observed + predicted-undetected) + curve, shared y-axis
* `figures/presentation/SRK_allele_accumulation_species_observed.png` / `_predicted.png` — slide build-up frames
...
> As of 2026-05-07 the script reads `SRK_individual_BL_assignments.tsv` (Step 13) and tests at three levels with the same split-filter design as Step 15 (species = 335 ingroup; EO/BL = 325 BL-assigned).
...
## TP1 — Mating-pool Functionality {#TP1}

> Requires the allele count matrix (Step 11), the inferred tetraploid zygosity table (Step 12), the BL assignments (Step 13), and the sampling metadata. The pre-existing erosion barplots from Steps 15–16 already document depletion against the species optimum; TP1 takes that as given and asks a different question.

**Scripts:**

1. `SRK_TP1_compatibility_metrics.py` — computes per-EO and per-BL metrics (including bootstrap 95 % CIs on P_compat **and the Depletion Index** `DI = 1 − k / k_species`), writes `Tables/SRK_EO_allele_richness.tsv`
2. `SRK_P_compat_traffic_light.R` — stakeholder-facing red/amber/green priority figure (full + blank variants); the dedicated random-mating-viability diagnostic foregrounded in the conservation report and presentations
3. `SRK_depletion_ranking.R` — **conservation-ranking figure** placing each group on P_compat × DI axes with quadrants labelled by the three breeding strategies in `Tables/SRK_breeding_strategies.csv` (HEALTHY / INFORMED BREEDING (frequency skew) / INFORMED BREEDING + ALLELE INJECTION (preventive / urgent)). Two panels (`_observed` uses sample-size-corrected k; `_predicted` uses the MM asymptote) × three variants (`_blank` zones only, `_EOs` focal EOs only, `_all` BLs + EOs) = 6 figures for layered presentation builds
...
**Inputs:**
- `Tables/SRK_individual_allele_genotypes.tsv` — Step 11 count matrix
- `Tables/sampling_metadata.csv` — per-individual EO assignment
- `SRK_individual_BL_assignments.tsv` — Step 13 BL labels
- `Tables/SRK_individual_zygosity.tsv` — Step 12 inferred tetraploid genotype classes (the metrics script applies the same inference rule directly from the raw matrix; the zygosity TSV is the validated reference)

### What TP1 measures — is the depleted population still functioning?

TP1 was reframed on 2026-05-22. The previous formulation (`prop_optimum × Ne/N_alleles`) repeated, in a scatter, what the Step 15 erosion barplots already show: how depleted each population is relative to the species-level optimum (≈ 58 alleles). The reframed TP1 asks the **complementary** question:

> *Given that this population is depleted, is its mating pool still functioning, and what conservation intervention does the data support?*

Two axes diagnose mating-pool function:

- **x = evenness J (Shannon H / ln k)** — *how evenly are the surviving alleles held?* J = 1 at NFDS equilibrium; J → 0 as one allele dominates (drift signature).
- **y = P_compat — the compatible-pair fraction** — *what fraction of random plant pairs are mating-compatible under tetraploid sporophytic SI?* A value of 0.40 means roughly 40 % of random pairs in the population can produce seed. Computed exactly from the inferred allele frequencies (formula below) as a deterministic closed-form value; uncertainty is quantified by a bootstrap 95 % CI (1000 resamples of individuals with replacement) reported alongside each point estimate. Directly demographic — translates to expected fraction of compatible crosses.

Point size encodes rarefied k (richness at N = 30, 1000 perms) so depletion remains visible without dominating the figure.

### How tetraploidy is handled

LEPA is tetraploid (2n = 4x). Amplicon SRK sequencing recovers ≤ 4 allele copies per individual but often returns fewer because of allele drop-out. Step 12 (`SRK_zygosity_from_genotype.R`) infers the full tetraploid genotype with this rule:

| Distinct alleles observed | Inferred tetraploid genotype | Logic |
|---|---|---|
| 1 | AAAA | fill to 4 copies of A |
| 2, dominant ≥ 2× minor | AAAB | 3 + 1 |
| 2, roughly equal | AABB | 2 + 2 |
| 3 | AABC | 2 + 1 + 1 |
| 4 | ABCD | 1 + 1 + 1 + 1 |

The metrics script applies this same rule before counting allele copies, so every individual contributes exactly 4 inferred copies. The per-EO denominator for allele frequencies is C = 4N.

### Compatibility formula

Sporophytic SI, tetraploid stigma (up to 4 distinct alleles), diploid pollen (up to 2 distinct alleles), co-dominance. Pollen is rejected if **any** of its 2 alleles matches **any** of the stigma's 4. The exact strict-SI formula under multinomial allele sampling is

$$ P_\text{compat strict} \;=\; \sum_{a, b} p_a\, p_b\, \bigl(1 - p_a - p_b \cdot \mathbb{I}[a \ne b]\bigr)^4. $$

Equal-frequency benchmark: $1 - 8/k$ (much harsher than the diploid $1 - 2/k$ — tetraploid SI requires roughly four times the allelic richness to reach the same compatibility level).

### SI leakage and the C3 Stage 5 connection

At leakage rate L ∈ [0, 1], a fraction L of would-be-incompatible matings succeed anyway (selfing or SI-leaky outcrossing):

$$ P_\text{compat}(L) \;=\; P_\text{compat strict} + L \cdot \bigl(1 - P_\text{compat strict}\bigr). $$

The script reports P_compat at L ∈ {0, 0.10, 0.25, 0.50}. An empirical estimate `L̂ = prop_AAAA / 3.5` is also computed per group: AAAA cannot arise under strict outcrossing SI, so observed AAAA reflects historical selfing or sib-crossing. The 3.5 divisor is a population-averaged tetrasomic correction (AABB self → AAAA in ~1/6 of offspring; AAAB self → AAAA in ~1/2; averaged over the AABB/AAAB mix typical of the data, the effective correction is ~3–4). `L̂` is an **upper bound** because some apparent AAAA arises from amplicon under-recovery.

### Inheritance-mode caveat

The exact P_compat formula assumes **tetrasomic** inheritance (random pairing across all four chromosomes — autotetraploid-like). If LEPA's SRK locus shows **disomic** inheritance (two homoeologous subgenomes segregating independently), the system behaves more like two superimposed diploid SI systems, which is generally more permissive — current P_compat is then a conservative lower bound. The formula should be revisited if cytology or segregation data establish disomic inheritance at SRK.

### Quadrants map directly to conservation actions

| Region | Reading | Intervention |
|---|---|---|
| top-right (high J, high P_compat) | functioning, even — NFDS still works | **MONITOR** + augment for long-term sustainability |
| top-left (low J, high P_compat) | mating works despite frequency skew | **AUGMENT** to restore evenness |
| bottom-right (high J, low P_compat) | evenly held but too few alleles | **AUGMENT URGENTLY** (too few compatible mates) |
| bottom-left (low J, low P_compat) | skewed and depleted — mating pool collapsed | **BIOBANK + RESTORE** |

**TP1 thresholds (adjustable at top of R script):**
...
Per-EO inclusion threshold is **N ≥ 15 individuals** (smaller EOs lack power for k_rarefied). All five BL aggregates are always plotted.

### Random-mating viability — traffic-light figure

- `figures/SRK_P_compat_traffic_light_EO.{png,pdf}` — **stakeholder-facing red/amber/green priority figure**. Six focal EOs ranked worst-to-best by strict-SI P_compat, with bootstrap 95 % CIs as horizontal error bars and BL colour squares on the left edge. The dedicated answer to *"is random mating still viable?"* — used in the LEPA report and in conservation presentations.
- `figures/SRK_P_compat_traffic_light_EO_blank.{png,pdf}` — companion blank variant (zones + EO labels + BL strip + legend; no data drawn) for the predictions slide.

### Depletion Index and the conservation-ranking figure

`SRK_depletion_ranking.R` produces a second 2D figure that aligns directly with the conservation decision the team has to make. Axes:

- **x = Depletion Index (DI)** = `1 − k / k_species`, where `k_species = 59` (Step 15 MM consensus). DI = 0 means at species equilibrium; DI = 1 means no alleles retained. Two flavours are written to the metrics TSV:
  - `DI_observed` uses `k_rarefied30` (sample-size-corrected), so EOs / BLs with different N are directly comparable. The conservative read.
  - `DI_predicted` uses the per-group MM asymptote `k_predicted_MM` from Step 15. It represents the upper bound on richness achievable from existing sampling without further intervention — gaps between observed and predicted DI flag populations where additional sampling would likely recover more alleles.
- **y = compatible-pair fraction** under strict tetraploid SI (`P_compat_L0`), with the bootstrap 95 % CI drawn as a vertical whisker.

Quadrants are labelled with the three breeding strategies defined in `Tables/SRK_breeding_strategies.csv`:

| DI | P_compat | Strategy |
|---|---|---|
...
Two panels are rendered (`_observed` and `_predicted`), each in three layered variants for stakeholder presentations:

- `figures/SRK_depletion_ranking_{observed,predicted}_blank.{png,pdf}` — zones + labels only (predictions slide)
- `figures/SRK_depletion_ranking_{observed,predicted}_EOs.{png,pdf}` — focal EOs only (mid-build; EOs are the management units)
- `figures/SRK_depletion_ranking_{observed,predicted}_all.{png,pdf}` — BLs + EOs together (final reveal)

In the current dataset every group except EO27 falls in one of the two ALLELE INJECTION quadrants. EO27 sits in the INFORMED BREEDING (frequency skew) quadrant on the `_predicted` panel and at the threshold on `_observed`. The observed-vs-predicted gap is largest for EO27 and BL4 (substantial sampling-driven contribution to apparent depletion) and smallest for EO70 (near-asymptote — depletion is real).

### Outputs

- `Tables/SRK_EO_allele_richness.tsv` — 5 BL rows + 6 EO rows × 31 columns: identifiers (`level`, `group`, `BL`, `N`), depletion (`k_species`, `k_predicted_MM`, `DI_observed`, `DI_predicted`), genotype mix (`geno_mix`, `prop_AAAA`, `L_hat_from_AAAA`, `raw_copy_recovery`), richness (`k_observed`, `k_rarefied30_mean/sd`), evenness (`shannon_H`, `evenness_J`, `chi2_uniform_p`, `P_compat_uniform_4x`), and the leakage ladder (`P_compat_L0/L0.10/L0.25/L0.50` each with `_lo/_hi` bootstrap CIs)
- `Tables/SRK_breeding_strategies.csv` — three-row reference table (Random pollination / Informed breeding / Informed breeding + allele injection) describing how parents are paired and what each strategy produces; provides the vocabulary used in the depletion-ranking quadrants
- Six PNG + six PDF depletion-ranking panels (observed/predicted × blank/EOs/all)
- One PNG + one PDF traffic-light figure (`SRK_P_compat_traffic_light_EO`) + matching `_blank` variant

### Headline result (current dataset)

Traffic-light: **EO70 is the only RED population** (P_compat ≈ 0.08, well below the 0.20 failure threshold); EO67 / EO18 / EO76 are AMBER; EO25 / EO27 sit at the GREEN border. Depletion ranking: every group except EO27 falls in one of the two ALLELE INJECTION quadrants. EO27 sits in the INFORMED BREEDING (frequency skew) quadrant on the `_predicted` panel and at the threshold on `_observed`; the observed-vs-predicted gap is largest for EO27 and BL4 (further sampling would likely recover more alleles) and smallest for EO70 (near-asymptote — depletion is real).
...
* Scripts:
  - `SRK_allele_sharing_EOs.py` — UpSet plots + pairwise heatmaps (EO + BL)
  - `SRK_allele_eulerr_BLs.R` — area-proportional 5-set Euler diagram of BL allele sets (stakeholder-facing companion to the BL UpSet; requires the R `eulerr` package)
* Environments: `polyploid-model` (Python: pandas, numpy, matplotlib) and base R + `eulerr`
* Execution (both scripts; regenerate together as the dataset grows):
...
* `figures/SRK_allele_eulerr_BLs.{png,pdf}` — **NEW (2026-05-22)**, area-proportional 5-set Euler diagram of BL allele sets. Same data as the BL UpSet but rendered as familiar Venn-style ellipses for stakeholder presentations. Produced by the R companion script (`SRK_allele_eulerr_BLs.R`).
...
| `SRK_individual_BL_assignments.tsv` | [Step 13](#BLintegration) | EO and BL assignments (325 BL-assigned of 335 ingroup individuals) |
...
| `BL_PALETTE` | Set1 (from `srk_bl_constants.R`) | `BL1=#984EA3`, `BL2=#377EB8`, `BL3=#E41A1C`, `BL4=#FF7F00`, `BL5=#4DAF4A` |
...
**BL-stratified design.** EOs are ordered by parent BL (total habitat area, then within-BL connectivity, largest at top) then by ascending mean Drift_index within BL; y-axis labels are coloured by parent BL (Set1 palette). The BL panel pools all BL-assigned individuals into 5 lineage rows, ordered by total habitat area then within-BL connectivity (BL4, BL5, BL3, BL1, BL2 — largest at top). The TP2 AAAA threshold (30%) is marked on both panels.
...
**`SRK_GFS_reproductive_effort` (page 1, EO panel):** Each horizontal bar represents one EO with N ≥ 5. EOs are ordered by parent BL (total habitat area, then within-BL connectivity, largest at top) then by ascending mean Drift_index within BL; y-axis labels are coloured by parent BL using the Set1 palette. Segments are coloured by GFS class (red = AAAA through blue = ABCD), stacked left to right from lowest to highest GFS. The dashed vertical line marks the TP2 AAAA threshold (30%): bars where the red (AAAA) segment extends past this line indicate a TP2 breach. Annotations to the right show proportion supporting (%), count (n / N), and mean GFS.

**`SRK_GFS_reproductive_effort` (page 2, BL panel):** Same layout for the 5 BLs, ordered by total habitat area then within-BL connectivity (BL4, BL5, BL3, BL1, BL2 — largest at top). All 325 BL-assigned individuals are aggregated into their respective BL rows; y-axis labels are coloured by BL.
...
# Phase 5: Testing S-allele Hypotheses {#phase5}

## Scientific Approach {#phase5-approach}

Phase 5 translates the sequence-based allele bins defined in Phase 2 into testable hypotheses about recognition specificity, using controlled crosses between genotyped individuals to validate whether sequence similarity predicts functional SI identity.
...
Step 26 is split into five scripts (26a–26e):

- **Step 26a — Cross-Brassicaceae Variability Landscape** (`srk_variability_landscape.py`): builds a combined alignment of the LEPA representative SRK alleles + 22 *Brassica rapa*/*B. oleracea* SRK references + 10 *Arabidopsis lyrata*/*A. halleri* SRK references, applies an identical sliding-window **Shannon-entropy** scan to each species, calls per-species HV regions at `mean + 1×SD` of the smoothed profile, and runs a 10 000-permutation test of pairwise HV-region overlap (LEPA↔Brassica, LEPA↔Arabidopsis, Brassica↔Arabidopsis). The 12 SCR9-contact residues from Ma et al. 2016 (*B. rapa* eSRK9-SCR9 crystal structure, PDB 5GYY) are mapped to LEPA alignment columns and overlaid as independent structural validation. The **66 canonical LEPA HV columns** are written to `SRK_LEPA_HV_positions.tsv` as the **canonical input** for downstream analyses. In the current post-QC dataset, the cross-genus HV-overlap permutation is no longer significant for LEPA pairs (LEPA ↔ Brassica *p* = 0.18; LEPA ↔ Arabidopsis *p* = 0.16); Brassica ↔ Arabidopsis remains highly significant (*p* < 0.0001) — interpreted as drift-eroded standing variation at LEPA HV columns. **Rebuild chain** (re-run every time Step 10 re-runs): `srk_fetch_reference_alleles.py` → `pad_representatives.py` → `mafft --add` → `srk_brassica_hv_mapping.py` → then `srk_variability_landscape.py`.

- **Step 26b — Allele Hypothesis Testing and Crossing Design** (`srk_allele_hypotheses.py`): reads `SRK_LEPA_HV_positions.tsv` (overrides any internal HV scan), computes HV-only pairwise distances on those columns, detects phylogenetic classes by UPGMA clustering (Class II = Allele_055 in the current dataset), builds the synonymy network (8 named synonymy groups of HV-identical alleles + Synonymy_test bridges; 27 effective bins from 58 defined alleles), and generates the **Incompatible / Synonymy_test / Compatible_within / Compatible_cross** crossing plan.

- **Step 26c — Synonymy-Group Collapse Test** (`srk_wgroup_collapse_test.py`, optional diagnostic): asks how much of LEPA's low Shannon entropy is attributable to synonymy-group sequence redundancy (multiple HV-identical alleles) versus genuine residue impoverishment.

- **Step 26d — Per-BL Entropy Decomposition** (`srk_perBL_entropy_test.py`, optional diagnostic): partitions LEPA HV-column residue diversity across the five BLs and compares to Brassica + Arabidopsis dominant residues, distinguishing **drift on a shared LEPA ancestral pool** (every BL fixes the same Synonymy group 1 alleles independently) from pan-Brassicaceae selection convergence. The verdict in the current dataset: drift, not selection convergence.

- **Step 26e — Hypothesis-Testing Cross Plan Generator** (`srk_cross_plan.py`): translates the Step 26b categorisation into a phased experimental protocol (H0 SI validation; H1a within-Class baseline; H1b between-Class baseline; H2 synonymy-bin tests; H3 hidden-bin tests via heterozygous donors + paired controls).

The current dataset contains 335 ingroup individuals; the AAAA-genotype subset is the ideal cross parents because their pollen carries a single unambiguous SCR identity, making cross outcomes directly interpretable.
...
* Scripts: `srk_variability_landscape.py` (Step 26a), `srk_allele_hypotheses.py` (Step 26b)
* Helper scripts:
...
# Step 26a — variability landscape
...
**Why the rebuild chain is non-optional:** the LEPA allele representatives define the *columns* of the combined alignment that downstream Step 26 scripts treat as canonical (HV column indices in `SRK_LEPA_HV_positions.tsv`, SCR9-contact mappings in `SRK_brassica_hv_mapping.tsv`). Re-running Step 10a with a different `N_ALLELES` produces a different representative set, shifting `mafft --add` column coordinates. Skipping the rebuild would silently mix Step 26 outputs against incompatible coordinate systems — a hard-to-debug failure mode. **Always rebuild in this order: pad → mafft --add → hv_mapping → Step 26a.**

#### Key Parameters

**Step 26a — `srk_variability_landscape.py`:**
...
**Step 26b — `srk_allele_hypotheses.py`:**

| Parameter | Default | Description |
|-----------|---------|-------------|
| `DOMAIN_REGION` | (31, 430) | S-domain alignment columns; must match Step 10 |
| `LEPA_HV_POSITIONS_FILE` | `SRK_LEPA_HV_positions.tsv` | Canonical HV columns from Step 26a (overrides legacy internal scan) |
...
|------|------|-------------|
| `SRK_combined_alignment.fasta` | mafft --add | LEPA + Brassica + Arabidopsis SRK references (input to Step 26a) |
| `SRK_brassica_hv_mapping.tsv` | helper script | Ma 2016 SCR9-contact residues → LEPA columns (input to Step 26a) |
| `SRK_protein_allele_representatives.fasta` | [Step 10](#SRKalleledist) | LEPA aligned representatives (input to Step 26b) |
| `SRK_individual_zygosity.tsv` | [Step 12](#zygosity) | Per-individual tetraploid genotype pattern (input to Step 26b) |
| `SRK_individual_allele_table.tsv` | [Step 11](#SRKfunctionalprotgenotype) | Individual → allele table (input to Step 26b) |
| `SRK_LEPA_HV_positions.tsv` | Step 26a | Canonical HV columns (input to Step 26b) |
...
| `SRK_LEPA_HV_positions.tsv` | **66 canonical LEPA HV columns** (consumed by Step 26b) |
...
## Hypothesis-Testing Cross Plan (Step 26e) {#crossplan}

> Requires `SRK_AAAA_cross_design_HV.tsv`, `SRK_synonymy_groups.csv`, `SRK_HV_allele_distances.tsv`, and `SRK_functional_allele_groups.tsv` from [Step 26b](#alleleclusters); `SRK_individual_BL_assignments.tsv` from [Step 13](#BLintegration); `SRK_individual_zygosity.tsv` from [Step 12](#zygosity); `SRK_individual_allele_table.tsv` from [Step 11](#SRKfunctionalprotgenotype). Run after Step 26b has completed.

### Overview

Step 26e is the **operational deliverable** of Phase 5: it translates the bioinformatics-predicted compatibility categories (Incompatible / Synonymy_test / Compatible_within / Compatible_cross) into a phased experimental protocol that field biologists can execute. The plan structures crossing experiments around five nested hypotheses (H0–H3) that progressively validate the SI system, calibrate the seed-yield scale, test bin-boundary predictions, and extend coverage to "hidden" bins lacking homozygous representatives. Each cross row in the output TSVs is fully traceable — Step 26e never invents pairings; it only translates upstream pipeline outputs into actionable experimental designs.

### Rationale — why this step is critical

The cross-design table from [Step 26b](#alleleclusters) (`SRK_AAAA_cross_design_HV.tsv`) lists 13 366 candidate AAAA × AAAA pairs categorised by sequence prediction (Incompatible / Synonymy_test / Compatible_within / Compatible_cross). But not every candidate is interpretable: Synonymy_test crosses where one parent is heterozygous would be confounded by polyploid pollen segregation, and 29 of the 63 sequence bins lack any AAAA representative at all. Step 26e enforces the genotype constraints required to produce *clean*, *interpretable* tests of each sequence-based prediction — and packages them into a coherent experimental sequence.

Without Step 26e, two reviewer-facing problems remain:
...
**Axis 1 — Sequence-based cross category** (from Step 26a + 26b):
...
| Per-individual allele composition | [Step 12](#zygosity) | `SRK_individual_zygosity.tsv` (`Allele_composition` column) | Authoritative source for which allele is the major (3-copy) vs minor (1-copy) in heterozygous genotypes. Step 26e reads this string directly, not the per-protein `SRK_individual_allele_table.tsv` |
...
* Execution (run from project root, after Step 26b):
...
| File | From | Description |
|------|------|-------------|
| `SRK_AAAA_cross_design_HV.tsv` | [Step 26b](#alleleclusters) | Pre-computed cross category for all AAAA × AAAA pairs |
| `SRK_synonymy_groups.csv` | [Step 26b](#alleleclusters) | Synonymy-group membership per allele |
| `SRK_HV_allele_distances.tsv` | [Step 26b](#alleleclusters) | Full 63 × 63 HV-distance matrix |
| `SRK_functional_allele_groups.tsv` | [Step 26b](#alleleclusters) | Allele → Class I / Class II |
...
The combined H2 + H3 outcomes produce a **validated functional S-allele table** that supersedes the sequence-only allele bin definitions and serves as the input to operational seed-orchard design (downstream of [Step 27](#crossresults)).

---

## Cross Result Analysis {#crossresults}

> Requires `SRK_functional_allele_groups.tsv` and `SRK_AAAA_cross_design_HV.tsv` from [Step 26](#alleleclusters), plus a completed cross results file. Run after crossing experiments have been conducted and seed yields recorded.

### Overview

This step reads completed cross records — a file with one row per cross, containing the mother and father plant IDs and a seed yield column — and assigns each cross to a Incompatible, Synonymy_test, Compatible_within, or Compatible_cross category using the HV-based class structure from Step 26. It then tests whether category predicts seed yield using Kruskal-Wallis and pairwise Mann-Whitney U tests, and produces a figure showing seed yield distributions and success rates per category.
...
| File | Description |
|------|-------------|
| `SRK_functional_allele_groups.tsv` | Allele class assignments from Step 26 |
| `SRK_AAAA_cross_design_HV.tsv` | Pairwise cross design with Incompatible / Synonymy_test / Compatible_within / Compatible_cross categories from Step 26 |
...
## Per-Individual SI System Status (Step 22) {#sistatus}

> **Question answered:** *What is the status of the SI system?* — at the individual, bottleneck-lineage (BL), and population (EO) levels.

### Overview

Step 22 reconstructs, for every ingroup individual, how many of the four expected tetraploid SRK copies produce a functional protein vs. carry a premature stop codon. It re-aggregates the per-haplotype `OK` / `REMOVED` calls that Step 7 (`translate_filter_align_AA.py`) writes to the per-library `_frame1_stopcodon_log.tsv` files and that the rest of the pipeline drops before genotyping. **Phase 1–2 outputs (canonical 49-allele catalogue, zygosity, allele matrix) are unchanged**; Step 22 sits at the start of Phase 4 and reads existing files only.

### Biological Rationale

Sporophytic SI in tetraploid *Lepidium papilliferum* requires at least one functional SRK haplotype to participate in pollen rejection. With four copies of the locus, an individual carrying:

- **4 functional copies → SI** (fully self-incompatible)
- **1–3 non-functional copies → pSI** (partially SI / leaky)
- **4 non-functional copies → SC** (self-compatible escape; same population biology as the seven candidates in `SRK_SI_escape_candidates.csv`)

Distinguishing these states at the population level is the **first question** in the workflow that links molecular SRK variation to the demographic / reproductive-success outcomes being explored in the LEPA report.

### Categorisation rule

Step 22 has two sub-steps. **Step 22a** (`SRK_null_allele_assignment.py`) aligns every `REMOVED` haplotype's AA sequence to the 49 canonical functional allele representatives via `mafft --add --keeplength` and computes p-distance. Confidence flag: `high` if `AA_distance ≤ 0.005` AND `n_stops ≤ 3`; `medium` if `AA_distance ≤ 0.05` AND `n_stops ≤ 10`; `low` otherwise (chimeric Canu artefact, no biological signal). **Step 22b** (`SRK_individual_SI_status.py`) then counts only the high+medium confidence broken haplotypes against the four-copy tetraploid expectation:

| `SI_status` | Criterion | Notes |
|---|---|---|
| **SI**   | `copies_NF == 0`  OR  `n_REMOVED_real < 2`  | Real broken haplotypes only; chimeric REMOVED haps excluded from the signal pool. |
| **pSI**  | `1 ≤ copies_NF ≤ 3`  AND  `n_REMOVED_real ≥ 2` | `pSI_confidence = high` if `frac_NF ≥ 0.25`, `low` otherwise. |
| **SC**   | `copies_NF == 4`                            | Every functional slot is a confidently-assigned broken allele. |
| **Insufficient_data** | `n_OK + n_REMOVED_real < 4`        | Most haplotypes were chimeric or the individual produced no Canu assembly — flagged for re-sequencing. |

`copies_NF = round(4 × n_REMOVED_real / (n_OK + n_REMOVED_real))` after excluding the `SRK_BEA` Brassica reference haplotypes.

### Bioinformatics

* Script (Step 22a — broken-allele identity): `SRK_null_allele_assignment.py`
* Script (Step 22b — SI categorisation): `SRK_individual_SI_status.py`
* Script (figures): `SRK_SI_status_figures.R`
...
| `all_Library*_*_frame1_AA_raw.fasta` (×9) | AA sequences for every haplotype (Step 22a). |
...
| `figures/SRK_SI_status_species_full.png` / `_robust.png` | Species-level stacked bar — `_full` (all 401 ingroup, 7-tier incl. pSI_low + Insufficient_data) and `_robust` (drops Insufficient_data + pSI_low). |
| `figures/SRK_SI_status_by_BL_full.png` / `_robust.png` | Per-BL stacked %. Same `_full` / `_robust` split. |
| `figures/SRK_SI_status_by_EO_full.png` / `_robust.png` | Per-EO stacked %, facet-grouped by BL with within-BL ordering by ascending drift index. Same `_full` / `_robust` split. |
...
|---|---:|---:|
| SI                | 247 | 61.6 % |
| pSI · high conf.  | 10  | 2.5 % |
| pSI · low conf.   | 5   | 1.2 % |
| SC                | 1   | 0.2 % |
| Insufficient data | 138 | 34.4 % |

The molecular SI machinery is broadly intact across the species. pSI carriers are sparse and geographically diffuse across all five BLs. **The single confirmed SC individual is `Library010_barcode53` in EO76 / BL3**, the same population that also carries the heaviest Insufficient_data load (n = 56 of EO76's 99 samples — flagged for re-sequencing in `Tables/SRK_samples_for_redo.tsv`). EO76 remains the SI → SC transitional hotspot.

## Null-Aware Tetraploid Genotype Rebuild (Step 23) {#nullaware}

> Propagates Step 22 SI status into the canonical genotype tables so every downstream population-genetic metric reflects the broken-copy reality.

### Overview

Step 23 rebuilds the per-individual tetraploid genotypes so that **partial-SI (pSI) individuals carry explicit `Allele_NULL` copies** and **SC individuals appear in the canonical set with all-null genotypes**. The original Phase-1–2 tables stay frozen as a functional-only reference; the null-aware tables sit alongside with a `_with_nulls` suffix.

### Biological motivation

The canonical genotype table counts only functional SRK copies and pads under-recovered individuals to four slots by homozygosity assumption (Step 12). For SI individuals this is correct. For pSI individuals the padding fabricates functional copies that biologically do not exist; for SC individuals the genotype is missing entirely because they fail at Step 9. Step 23 propagates Step 22's per-individual SI status into a parallel null-aware genotype matrix so downstream Phase-3 metrics can be recomputed with explicit broken alleles. The original Phase-1–2 tables stay frozen as a functional-only reference; the null-aware tables sit alongside with a `_with_nulls` suffix.

### Categorisation rule

| Source SI_status | Action | Rationale |
|---|---|---|
| `SI` (n = 247) | Pad to 4 functional copies; `Allele_NULL = 0`. | Truly SI — no nulls. |
| `pSI` + `pSI_confidence == low` (n = 5) | Promote to SI; pad as above. | `frac_NF < 0.25` too dilute to commit nulls confidently. |
| `pSI` + `pSI_confidence == high` (n = 10) | Scale functional counts down to `copies_functional`; `Allele_NULL = copies_nonfunctional`. | Genuine partial SI — broken copies enter allele-frequency denominators. |
| `SC` (n = 1) | All four slots → `Allele_NULL`. | Library010_barcode53, EO76 / BL3. |
| `Insufficient_data` (n = 138) | Keep in canonical set if reached genotyping; flag for re-sequencing either way. | Genotype call is data-thin. |

### Null-aware GFS formula

\[
\mathrm{GFS}_{\text{null-aware}} = \mathrm{GFS}_{\text{func}} \times \frac{n_{\text{func}}}{4}, \qquad
\mathrm{GFS}_{\text{func}} = 1 - \frac{\sum_k n_k(n_k - 1)}{n_{\text{func}}(n_{\text{func}} - 1)}
\]

with $n_{\text{func}} = 4 - N_{\text{null}}$. Produces a 12-tier ordering: `0000 / A000 / AA00 / AAA0 / AAAA → 0.000`; `AAAB / AB00 / AAB0 → 0.500`; `AABB → 0.667`; `ABC0 → 0.750`; `AABC → 0.833`; `ABCD → 1.000`.

### Scripts

* `SRK_genotype_null_alleles.py` — builds the null-aware genotype + zygosity tables and the redo list.
* `SRK_population_genetic_summary_with_nulls.R` — Step 14b: null-aware pop-gen with new columns `Frac_nonfunctional_alleles`, `Mean_null_copies`, `Prop_pSI`, `N_SC`.
* `SRK_TP1_compatibility_metrics_with_nulls.py` — Step 17b: null-aware P_compat (`Allele_NULL` enters the frequency vector; heuristic but conservative).
* `SRK_individual_GFS_with_nulls.R` — Steps 19b + 20b: null-aware GFS with the formula above.

### Outputs

| File | Content |
|---|---|
| `Tables/SRK_individual_allele_genotypes_with_nulls.tsv` | 336 individuals × 49 functional alleles + Allele_NULL + Genotype_class_flag. Rows sum to 4. |
| `Tables/SRK_individual_zygosity_with_nulls.tsv` | 12-tier genotype labels (`AB00`, `0000`, …) + N_distinct_functional + N_null_copies + SI_status. |
| `Tables/SRK_samples_for_redo.tsv` | 138 Insufficient_data individuals, prioritised by `n_haps_total`. |
| `SRK_population_genetic_summary_with_nulls.tsv` / `_BL_with_nulls.tsv` / `.pdf` | Step 14b outputs. |
| `Tables/SRK_EO_allele_richness_with_nulls.tsv` | Step 17b: null-aware TP1 metrics with bootstrap CIs at L ∈ {0, 0.10, 0.25, 0.50}. |
| `SRK_individual_GFS_with_nulls.tsv` / `SRK_EO_GFS_summary_with_nulls.tsv` / `SRK_BL_GFS_summary_with_nulls.tsv` | Steps 19b + 20b. |
| `figures/SRK_GFS_with_nulls_composition.png` / `_TP2_scatter.png` | Stacked-bar tier composition per BL + null-aware TP2 scatter. |

### Canonical vs null-aware deltas

| BL | Canonical P_compat (L=0) | Null-aware P_compat (L=0) | Δ |
|---|---:|---:|---:|
| BL1 | 0.068 | 0.091 | +0.023 |
| BL2 | 0.106 | 0.112 | +0.006 |
| BL3 | 0.394 | 0.406 | +0.012 |
| BL4 | 0.417 | 0.422 | +0.005 |
| BL5 | 0.472 | 0.481 | +0.009 |

Per-BL broken-allele frequencies are small (`frac_null` = 0.005 – 0.05), so the null-aware metrics **corroborate** the canonical Phase-3 conclusions rather than revise them. All five BLs remain CRITICAL on TP2 in both views.

## Forward-Time Inheritance Simulator (Step 24) {#inheritance}

> Tetraploid Wright–Fisher simulator that projects each BL forward in generation time from its empirical Step 23 state. The dynamic companion to Step 22's snapshot — answers *where* the species is heading on the SI → SC erosion axis.

### Overview

Step 24 simulates the **dynamic** SI → pSI → SC trajectory of each BL under drift, mutation, sporophytic SI rejection, selfing, inbreeding depression, and optional inter-BL migration, from the empirical Step 23 initial state.

### Model

Each individual = vector of 4 SRK haplotypes encoded as integers, with 0 = `Allele_NULL` and positive integers indexing functional alleles. Per generation:

1. **Mutation** — each copy → NULL with probability μ (default 1 × 10⁻⁴).
2. **Mate selection** — pick a mother uniformly at random; if she has ≥ 3 NULL stigma copies AND `U(0,1) < s`, she selfs (default `s = 0.5`). Otherwise she attempts outcrossing for up to 20 attempts.
3. **Sporophytic SI rejection** — pollen rejected if any functional pollen S-allele matches any functional stigma S-allele. **NULL alleles never trigger rejection**; NULL stigma copies never reject.
4. **Gamete formation** — tetrasomic random pairing with double-reduction probability α = 0.10.
5. **Inbreeding depression** — selfed offspring fitness = 1 − δ (default δ = 0.5); Wright–Fisher resampling weighted by fitness.
6. **Optional migration** — with probability *m* per individual, replace it with a random individual drawn from the other BLs' current populations.

### Scenarios

| Scenario | μ | s | δ | m | Ne_scaling | Conservation reading |
|---|---|---|---|---|---|---|
| baseline | 1e-4 | 0.5 | 0.5 | 0 | 1.0 | Current isolated state — no intervention. |
| rescue_low | 1e-4 | 0.5 | 0.5 | 0.001 | 1.0 | Light passive inter-BL gene flow. |
| rescue_high | 1e-4 | 0.5 | 0.5 | 0.01 | 1.0 | 10× rescue_low. |
| high_drift | 1e-4 | 0.5 | 0.5 | 0 | 0.5 | Further habitat loss stress test. |

### Bioinformatics

* `SRK_inheritance_simulator.py` — runs all scenarios × all BLs × `--n_replicates` × `--n_generations`. CLI flags: `--n_generations`, `--n_replicates`, `--mu`, `--scenarios`, `--seed`, `--quick`. Default 200 gens × 50 reps; production runs use 100 gens × 30 reps for tractable runtime.
...
| File | Content |
|---|---|
| `Tables/SRK_inheritance_trajectories.tsv` | Per (scenario × BL × replicate × generation): n_SI, n_pSI1, n_pSI2, n_pSI3, n_SC, mean_p_NULL. |
| `Tables/SRK_inheritance_time_to_sc.tsv` | First-passage time to 50 % SC frequency per (scenario × BL × replicate). |
| `figures/SRK_inheritance_pNULL_trajectories.png` | Per-BL p_NULL trajectories. |
| `figures/SRK_inheritance_SC_progression.png` | SC-frequency trajectories with IQR ribbons. |
| `figures/SRK_inheritance_time_to_sc.png` | Median + IQR time-to-50%-SC bars per BL × scenario. |

#### Headline Results (100 gens × 30 reps)

Median generations to 50 % SC frequency:

...
- **External reference FASTAs** (Brassica, Arabidopsis SRK references for Step 22) live in `FASTA/`.
- `Tables/sampling_metadata.csv` is the single canonical metadata file (symlink at `./sampling_metadata.csv` for legacy scripts).

**Phase 4 (Step 22) update:** `pad_representatives.py` now restricts LEPA representatives to ingroup-observed alleles only (49 of 55), removing outgroup contamination from the cross-Brassicaceae variability landscape analysis.

**Phase 5 (Steps 25–28 + null-aware 14b/17b/19b/20b reruns)** writes to `Tables/Phase4/` and `figures/Phase4/` with `step25a/25b/26/14b/17b/19b/20b/27/28_*` prefixes.
...
25. **Per-individual SI / pSI / SC reconstruction** – `SRK_individual_SI_status.py` re-aggregates per-haplotype OK / REMOVED calls to classify each individual as full SI, partial SI, self-compatible, or insufficient_data. `SRK_null_allele_assignment.py` augments this by mapping REMOVED haplotypes to their nearest functional allele via amino acid distance.
26. **Null-aware genotype rebuild** – `SRK_genotype_null_alleles.py` rewrites the allele × individual matrix so that partial SI individuals carry explicit `Allele_NULL` copies and self-compatible individuals are represented as `Allele_NULL × 4`. Triggers null-aware cascade re-runs of Steps 14b (population genetics), 17b (random-mating viability), and 19b + 20b (fitness scores + reproductive collapse risk).
27. **Forward-time tetraploid inheritance simulator** – `SRK_inheritance_simulator.py` simulates per-lineage trajectories under multiple scenarios (baseline / high_drift / rescue_high / rescue_low) starting from the empirical Step 26 genotypes.
28. **Donor ranking for allele injection** – `SRK_injection_donor_ranking.py` ranks candidate seed parents for allele-injection crosses to rescue specific lineages / populations.
...
- **Step 22c — Synonymy-Group Collapse Test** (`srk_wgroup_collapse_test.py`, optional diagnostic): asks how much of LEPA's low Shannon entropy is attributable to synonymy-group sequence redundancy (multiple HV-identical alleles) versus genuine residue impoverishment.

- **Step 22d — Per-BL Entropy Decomposition** (`srk_perBL_entropy_test.py`, optional diagnostic): partitions LEPA HV-column residue diversity across the five BLs and compares to Brassica + Arabidopsis dominant residues, distinguishing **drift on a shared LEPA ancestral pool** (every BL fixes the same Synonymy group 1 alleles independently) from pan-Brassicaceae selection convergence. The verdict in the current dataset: drift, not selection convergence.

- **Step 22e — Hypothesis-Testing Cross Plan Generator** (`srk_cross_plan.py`): translates the Step 22b categorisation into a phased experimental protocol (H0 SI validation; H1a within-Class baseline; H1b between-Class baseline; H2 synonymy-bin tests; H3 hidden-bin tests via heterozygous donors + paired controls).
...
## Per-Individual SI System Status (Step 25) {#sistatus}

> **Question answered:** *What is the status of the SI system?* — at the individual, bottleneck-lineage (BL), and population (EO) levels.

### Overview

Step 25 reconstructs, for every ingroup individual, how many of the four expected tetraploid SRK copies produce a functional protein vs. carry a premature stop codon. It re-aggregates the per-haplotype `OK` / `REMOVED` calls that Step 7 (`translate_filter_align_AA.py`) writes to the per-library `_frame1_stopcodon_log.tsv` files and that the rest of the pipeline drops before genotyping. **Phase 1–2 outputs (canonical 49-allele catalogue, zygosity, allele matrix) are unchanged**; Step 25 sits at the start of Phase 5 and reads existing files only.
...
Step 25 has two sub-steps. **Step 25a** (`SRK_null_allele_assignment.py`) aligns every `REMOVED` haplotype's AA sequence to the 49 canonical functional allele representatives via `mafft --add --keeplength` and computes p-distance. Confidence flag: `high` if `AA_distance ≤ 0.005` AND `n_stops ≤ 3`; `medium` if `AA_distance ≤ 0.05` AND `n_stops ≤ 10`; `low` otherwise (chimeric Canu artefact, no biological signal). **Step 25b** (`SRK_individual_SI_status.py`) then counts only the high+medium confidence broken haplotypes against the four-copy tetraploid expectation:
...
* Script (Step 25a — broken-allele identity): `SRK_null_allele_assignment.py`
* Script (Step 25b — SI categorisation): `SRK_individual_SI_status.py`
...
| `all_Library*_*_frame1_AA_raw.fasta` (×9) | AA sequences for every haplotype (Step 25a). |
...
## Forward-Time Inheritance Simulator (Step 27) {#inheritance}

> Tetraploid Wright–Fisher simulator that projects each BL forward in generation time from its empirical Step 26 state. The dynamic companion to Step 25's snapshot — answers *where* the species is heading on the SI → SC erosion axis.

### Overview

Step 27 simulates the **dynamic** SI → pSI → SC trajectory of each BL under drift, mutation, sporophytic SI rejection, selfing, inbreeding depression, and optional inter-BL migration, from the empirical Step 26 initial state.

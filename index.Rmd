...
## Step 12c — Data Quality Evaluation (end of Phase 2) {#sampleaudit}

### Overview

A formal data-quality gate at the end of Phase 2 that runs after Step 12 (zygosity) completes and before Step 13 (BL integration) begins. It asks **three sequential questions** and produces deliverables for both the bioinformatics record and the wet-lab team.
...
2. `SRK_TP1_compatibility.R` — renders the four diagnostic panels (EO + BL × strict + leaky)
3. `SRK_P_compat_traffic_light.R` — stakeholder-facing red/amber/green priority figure (full + blank variants); the dedicated random-mating-viability diagnostic foregrounded in the conservation report and presentations
...
To keep the figures readable for stakeholder presentations, EO and BL points are placed on separate panels, each at two leakage levels:

- `figures/SRK_TP1_compatibility_EO_strict.{png,pdf}` — EO panel, strict SI (L = 0)
- `figures/SRK_TP1_compatibility_EO_leaky.{png,pdf}` — EO panel, leaky SI (L = 0.25, close to empirical L̂ ≈ 0.18)
...
Each four-panel figure ships with a `_blank.png` companion (empty quadrants, no points) for presentation overlays.

### Depletion Index and the conservation-ranking figure

`SRK_depletion_ranking.R` produces a second 2D figure that complements the J × P_compat scatter — and aligns more directly with the conservation decision the team has to make. Axes:
...
- Four PNG + four PDF panels (J × P_compat, above)
- Four `_blank.png` zone-only variants
- Six PNG + six PDF depletion-ranking panels (observed/predicted × blank/BL/all)

### Headline result (current dataset, 2026-05-22)

- **Strict SI (L = 0):** EO70 sits alone in BIOBANK + RESTORE (P_compat = 0.08, J = 0.67, k_rare30 = 4.9 against a species optimum ≈ 58). EO67 also BIOBANK (P_compat = 0.23). EO18, EO76 in AUGMENT URGENTLY. EO25, EO27 in MONITOR. BL1 + BL2 in BIOBANK; BL3 borderline AUGMENT URGENTLY; BL4 + BL5 in MONITOR.
- **Leaky SI (L = 0.25):** EO18, EO76, BL3 rescued into MONITOR (the C3 Stage 5 signature). **EO70 and BL2 stay in BIOBANK under both views** — the cleanest signal that SI leakage cannot rescue these populations and seed banking is the priority intervention. EO67 and BL1 partially rescue into AUGMENT URGENTLY.

The rescue effect visible across the leaky-vs-strict comparison **is** C3 Stage 5: populations that look catastrophic under strict SI are demographically alive only because SI is leaky. The cost is homozygosity accumulation (prop_AAAA ranges 0.55–0.73 across the six focal EOs), which feeds Stage 2 of the cascade via the self-reinforcing loop.
...
|---|---|---|
| **SI**   | `copies_NF == 0`  OR  `n_REMOVED < 2`        | Single-REMOVED haps treated as Canu assembly noise. |
| **pSI**  | `1 ≤ copies_NF ≤ 3`  AND  `n_REMOVED ≥ 2`    | `pSI_confidence = high` if `frac_NF ≥ 0.25`, `low` otherwise. |
| **SC**   | `copies_NF == 4`                              | Robust — every haplotype independently carries a stop. |
| **Insufficient_data** | `n_OK + n_REMOVED < 4`           | Mostly individuals that never produced a Canu assembly. |

`copies_NF = round(4 × n_REMOVED / (n_OK + n_REMOVED))` after excluding the `SRK_BEA` Brassica reference haplotypes.

### Assembly-noise caveat

Canu can emit N > 4 haplotypes per individual (chimeric assemblies splice two real alleles and introduce spurious frameshifts; indel-rich low-coverage contigs shift the reading frame; allele-specific over-clustering yields multiple near-identical contigs). A single `REMOVED` call in an otherwise clean set is exactly what an assembly chimera produces, so the `n_REMOVED ≥ 2` filter reassigns those individuals to SI. The **SC class is unchanged and biologically robust** because every haplotype must independently carry a stop. The pSI class is reported with a confidence flag (`pSI_high` / `pSI_low`) so downstream users can filter.

### Bioinformatics

* Script (aggregator): `SRK_individual_SI_status.py`
* Script (figures): `SRK_SI_status_figures.R` (uses `srk_bl_constants.R` for `BL_ORDER` and `get_eo_order_within_bl()`)

#### Input Files

| File | Description |
|---|---|
| `all_Library*_*_frame1_stopcodon_log.tsv` (×9) | Per-haplotype OK/REMOVED calls from Step 7 (`translate_filter_align_AA.py`). |
| `Tables/SRK_data_quality_categories.tsv` | `Ingroup` flag + `EO_normalised` + `BL_inferred` (from Step 12c). |

#### Output Files

| File | Content |
|---|---|
| `Tables/SRK_individual_SI_status.tsv` | One row per ingroup individual: `n_haps_OK`, `n_haps_REMOVED`, `n_haps_total`, `frac_nonfunctional`, `copies_functional`, `copies_nonfunctional`, `SI_status` (SI / pSI / SC / Insufficient_data), `pSI_confidence` (high / low / blank). |
| `figures/SRK_SI_status_species_full.png` / `_robust.png` | Species-level stacked bar — `_full` (n = 401, 7-tier including pSI · low confidence and Insufficient_data) and `_robust` (n = 254, 5-tier SI / pSI_1nf / pSI_2nf / pSI_3nf / SC for biological-process readings). |
| `figures/SRK_SI_status_by_BL_full.png` / `_robust.png` | Per-BL stacked %, BLs in `BL_ORDER` (area-then-connectivity sort key). Same `_full` / `_robust` split. |
...
|---|---:|---:|
| SI                | 37  | 9.2 % |
| pSI · high conf.  | 209 | 52.1 % |
| pSI · low conf.   | 23  | 5.7 % |
| SC                | 8   | 2.0 % |
| Insufficient data | 124 | 30.9 % |

Only ~13 % of individuals with sufficient data carry a fully intact SI system. The species-wide dominance of high-confidence pSI is the headline result and likely contributes to the reproductive-success deficit observed at the demographic level. The eight SC individuals — clustered in EO76 (n = 3), EO29 (n = 2), EO18, EO70, EO97 (one each) — are the most biologically robust signal of ongoing SI breakdown.
...
The canonical genotype table counts only functional SRK copies and pads under-recovered individuals to four slots by homozygosity assumption (Step 12). For an SI individual this is correct. For a pSI individual the padding *fabricates* functional copies that biologically do not exist — apparent homozygosity is inflated, allele-frequency tails are depressed, and downstream metrics (P_compat, GFS, TP1, TP2) are biased optimistic. SC individuals are excluded from the canonical 335-individual set entirely.

### Categorisation rule (Sven, 2026-06-11)

| Source SI_status | Action | Rationale |
|---|---|---|
| `SI` (n = 37) | Pad to 4 functional copies; `Allele_NULL = 0`. | Truly SI — no nulls. |
| `pSI` + `pSI_confidence == low` (n = 23) | Promote to SI; pad as above. | `frac_NF < 0.25` too dilute to commit nulls. |
| `pSI` + `pSI_confidence == high` (n = 209) | Scale functional counts down to `copies_functional`; `Allele_NULL = copies_nonfunctional`. | Genuine partial SI — broken copies must appear in allele-frequency denominators. |
| `SC` (n = 8) | All four slots → `Allele_NULL`. Adds 7 individuals to canonical set. | Self-compatible escape — robust under Canu noise. |
| `Insufficient_data` (n = 124) | Keep in canonical set if reached genotyping; flag for re-sequencing either way. | Genotype call is data-thin but not nonsense. |
...
| File | Content |
|---|---|
| `Tables/SRK_individual_allele_genotypes_with_nulls.tsv` | 342 individuals × 49 functional alleles + Allele_NULL + Genotype_class_flag. Rows sum to 4. |
...
| `Tables/SRK_samples_for_redo.tsv` | 124 Insufficient_data individuals, prioritised by `n_haps_total`. |
...
### Bias correction summary (2026-06-11)

| BL | Canonical P_compat (L=0) | Null-aware P_compat (L=0) | Δ |
|---|---:|---:|---:|
| BL1 | 0.068 | 0.084 | +0.02 |
| BL2 | 0.106 | 0.137 | +0.03 |
| BL3 | 0.394 | 0.367 | -0.03 |
| BL4 | 0.417 | 0.358 | **-0.06** |
| BL5 | 0.472 | 0.392 | **-0.08** |

The functional-only baseline **overestimated** the mating-pool size for the larger BLs (BL3–BL5) by 0.03–0.08. Null-aware GFS shows **all five BLs CRITICAL** on TP2 (mean GFS < 0.667 AND prop_zero > 30 %), with 60–75 % of individuals in every BL falling to GFS = 0. The conclusion that LEPA is reproductively compromised at the molecular level is *strengthened*, not weakened, by the null-aware rebuild.
...
A two-layer model for the SI → pSI → SC process: Step 25 + the binomial-snapshot fit (Bin(4, p_BL) explains the within-BL distribution of broken copies; per-BL p̂ = 0.39–0.57) gives the **static** picture; Step 27 simulates the **dynamic** trajectory under drift, mutation, sporophytic SI rejection, selfing, inbreeding depression, and optional inter-BL migration.
...
#### Headline Results (100 gens × 30 reps, 2026-06-11)
...
**Three biological readings.** (1) Every BL crosses the 50 % SC threshold within ~25 generations under baseline — *L. papilliferum* is on a rapid SI → SC trajectory in the absence of intervention. (2) Halving effective N (high_drift) accelerates collapse by 4–7 generations for BL3–BL5. (3) **Passive inter-BL migration alone cannot rescue**, because donor BLs themselves carry high p_NULL — random migrants just average a partially-broken state across the species. **Targeted SI-mother / SI-father crosses (Step 22e cross plan filtered through the Step 25 SI table)** are the conservation lever that must be modelled in future scenario extensions (`rescue_targeted`, `rescue_targeted_seed`, `biobank_pause`).

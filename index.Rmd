...
21. [Allele Super-Group Clustering and Crossing Design](#alleleclusters) – Second-level UPGMA clustering of the allele bins defined in Phase 2 into super-groups based on S-domain p-distance, producing a prioritised crossing plan for AAAA individuals with cross categories W (within-bin), N (within-cluster), and P (between-cluster).
22. [Cross Result Analysis](#crossresults) – Statistical comparison of seed yield across W / N / P cross categories to test whether sequence-based allele bins predict functional recognition specificity.
...
**The experimental logic rests on three cross categories:**

- **W — within-bin** (same allele bin, S-domain distance = 0): pollen and pistil carry the same SRK allele. Under a functional SI system, these crosses should fail — the pistil recognises the pollen as self and rejects it. These are negative controls. Any seed set observed in W crosses is direct evidence of SI breakdown, consistent with Stage 5 of the Compatibility Collapse Cascade.

- **N — within-cluster** (different allele bins, same super-group): pollen and pistil carry alleles that are closely related in sequence but were defined as distinct bins. These crosses are the core of the hypothesis test — seed failure would indicate that sequence-similar alleles are functionally synonymous; seed set would confirm that they carry distinct recognition specificities.

- **P — between-cluster** (alleles from different super-groups): pollen and pistil carry alleles that are substantially divergent in sequence. These crosses are expected to succeed. They serve as positive controls establishing that the SI system is functional and that compatible pollen is being applied correctly.

**Why AAAA individuals?** Individuals homozygous for a single allele across all four genome copies (AAAA genotype; 56% of all sampled plants) are the ideal cross parents for this experiment. Their pollen carries a single SCR identity — the allele assignment is unambiguous, and cross outcomes can be interpreted without accounting for pollen genotype distributions. Crosses between two AAAA plants are therefore the cleanest test of allele-level recognition specificity available in a tetraploid system.

**Two-step analysis:** [Step 21](#alleleclusters) groups the allele bins into super-groups by second-level UPGMA clustering and generates the prioritised crossing plan. [Step 22](#crossresults) reads completed cross records and tests whether the W / N / P category predicts seed yield, providing the statistical confirmation needed to validate or refine the sequence-based allele definitions.

---

## Allele Super-Group Clustering and Crossing Design {#alleleclusters}

> Requires `SRK_protein_allele_representatives.fasta` from [Step 10](#SRKalleledist), `SRK_individual_GFS.tsv` from [Step 18](#GFS), and `SRK_individual_allele_table.tsv` from [Step 11](#SRKfunctionalprotgenotype). Run after Steps 10, 11, and 18 have completed.

### Overview

This step groups the allele bins defined in Phase 2 into super-groups by running a second-level UPGMA hierarchical clustering on pairwise S-domain p-distances between allele representative sequences. Each of the 105 AAAA individuals in the dataset is then assigned a bin and super-group, and all pairwise AAAA combinations are categorised as W, N, or P and ranked to produce a crossing plan. The result is a practical field guide: which crosses to prioritise, what to expect, and what each outcome means for the allele definitions.

The step also prints a distance summary that diagnoses the structure of the allele distance space — in particular, whether the alleles form a single tight cluster (requiring a finer `N_SUPERGROUPS` setting to resolve useful sub-groups) or whether natural breaks exist at the chosen resolution.

### Biological Rationale

The allele bin boundaries from Phase 2 are sequence-based hypotheses. The super-group clustering adds a second level of resolution: within each super-group, allele bins are more similar to each other than to bins in other super-groups. This provides a gradient of expected recognition similarity — from identical (W, same bin) through closely related (N, same super-group) to divergent (P, different super-group) — that maps directly onto experimental predictions for seed yield. The three-tier design maximises the information return from a limited crossing experiment: every cross tests either a hypothesis (N) or anchors the interpretation via a control (W or P).

Of the 55 allele bins currently defined, 22 have at least one AAAA representative in the dataset. Among these, 14 bins have two or more AAAA plants and can support the full three-tier crossing design (within-bin negative control, within-cluster hypothesis test, between-cluster positive control). The remaining 8 bins with a single AAAA individual can be used as cross partners but cannot provide a replicated within-bin incompatibility test. The 33 bins with no AAAA representatives require AAAB or AABB parents and are deferred to a future analysis extension.
...
| Parameter | Default | Description |
|-----------|---------|-------------|
| `N_SUPERGROUPS` | 8 | Number of super-groups; increase if most alleles collapse into one group (see distance summary in output) |
| `SUPERGROUP_THRESHOLD` | 0.10 | Alternative: p-distance cutoff used when `N_SUPERGROUPS = None` |
| `DOMAIN_REGION` | (31, 430) | S-domain columns used for distance; must match Step 10 |
| `CROSS_TSV` | None | Set to cross results filename to activate Part 2 (Step 22) |

**Tuning `N_SUPERGROUPS`:** The script prints a distance summary on each run. If the counts for `dist < 0.05`, `dist < 0.10`, and `dist < 0.20` are identical, the allele distance space is bimodal — most alleles form a tight cluster with no intermediate distances. In this case, increase `N_SUPERGROUPS` to 15–25 to force the algorithm to subdivide the cluster into experimentally manageable groups. Inspect `SRK_allele_cluster_figure.pdf` to choose a value where the dendrogram shows natural internal branching rather than an arbitrary cut through a flat cluster.
...
| `SRK_individual_GFS.tsv` | Per-individual GFS scores and genotype pattern (output of Step 18) |
| `SRK_individual_allele_table.tsv` | Individual → allele → copy count table (output of Step 11) |

#### Output Files

| File | Content |
|------|---------|
| `SRK_allele_supergroups.tsv` | One row per allele bin: super-group assignment, AAAA individual count, cross power rating |
| `SRK_AAAA_cross_design.tsv` | All AAAA × AAAA pairs ranked by category (W first, then N, then P), with allele IDs, super-group IDs, S-domain distance, and expected outcome |
| `SRK_allele_cluster_figure.pdf` | Left panel: UPGMA dendrogram of allele bins coloured by super-group. Right panel: bar chart of AAAA individuals per allele bin in the same leaf order, with dashed line at n = 2 |

#### Output File Descriptions

**`SRK_allele_supergroups.tsv`**

| Column | Description |
|--------|-------------|
| `Allele` | Allele bin name |
| `SuperGroup` | Super-group assignment (SG01–SGxx) |
| `N_AAAA` | Number of AAAA individuals carrying this allele |
| `CrossPower` | `full` (≥2 AAAA, full 3-tier design possible), `singleton` (1 AAAA, partner only), `none` (no AAAA) |

**`SRK_AAAA_cross_design.tsv`**

| Column | Description |
|--------|-------------|
| `Mother` / `Father` | Individual plant IDs |
| `Mother_allele` / `Father_allele` | Allele bin carried by each plant |
| `Mother_SG` / `Father_SG` | Super-group of each allele |
| `S_domain_dist` | Pairwise S-domain p-distance between the two alleles |
| `Category` | **W**, **N**, or **P** — the key column for cross prioritisation |
| `Expected` | Plain-language prediction for seed yield |

### Scientific Applications

* **Cross prioritisation**: N pairs (within-cluster hypothesis test) are the highest scientific priority — they directly test whether sequence-similar bins are functionally synonymous; P pairs confirm the SI system is working; W pairs confirm that the genotyping correctly identifies the allele
* **Allele bin refinement**: if N crosses consistently fail (no seeds), the current bin boundaries may be too fine and neighbouring bins could be merged; if N crosses succeed, the bins are well-delimited
* **Conservation breeding**: W crosses that yield seeds are direct evidence of SI breakdown and can be reported as Stage 5 Compatibility Collapse Cascade events in the individual plants tested

---

## Cross Result Analysis {#crossresults}

> Requires `SRK_allele_supergroups.tsv` and `SRK_AAAA_cross_design.tsv` from [Step 21](#alleleclusters), plus a completed cross results file. Run after crossing experiments have been conducted and seed yields recorded.

### Overview

This step reads completed cross records — a file with one row per cross, containing the mother and father plant IDs and a seed yield column — and assigns each cross to a W, N, or P category using the super-group structure from Step 21. It then tests whether category predicts seed yield using Kruskal-Wallis and pairwise Mann-Whitney U tests, and produces a figure showing seed yield distributions and success rates per category.

**To activate this step**, set `CROSS_TSV` in the settings section of `test_allele_definitions_from_crosses.py` to the filename of the completed cross records file, then re-run the script.

### Biological Rationale

The statistical test is a direct evaluation of the allele bin definitions: if the bins correctly capture functional recognition specificity, W crosses should show the lowest seed yield (same allele, rejected by SI), P crosses the highest (different super-group, fully compatible), and N crosses an intermediate or categorical pattern depending on whether the closely related bins are functionally synonymous or distinct. A significant Kruskal-Wallis result with the expected ordering (W < N ≤ P) validates the sequence-based bin boundaries. A result where W ≥ N suggests the bins may be over-split and closely related alleles share specificity. A result where W > 0 consistently indicates SI breakdown in the tested individuals.

### Bioinformatics

* Script: `test_allele_definitions_from_crosses.py` (Part 2, activated by setting `CROSS_TSV`)
* Environment: Python 3 with numpy, pandas, matplotlib, scipy, biopython
...
| File | Description |
|------|-------------|
| `SRK_allele_supergroups.tsv` | Allele super-group assignments from Step 21 |
| `SRK_individual_allele_table.tsv` | Individual → allele table, used to assign alleles to non-AAAA cross parents |
| Cross results file | One row per cross; must contain `Mother`, `Father`, and a seed count column (auto-detected: `Seeds`, `germplasmQuantityEstimate`, `seed_count`, or first numeric column) |

#### Output Files

| File | Content |
|------|---------|
| `SRK_cross_result_analysis.pdf` | Left panel: box + strip plot of seed yield by W / N / P category. Right panel: bar chart of cross success rate (% with seeds > 0) by category |
...
| Mann-Whitney U (pairwise) | Post-hoc comparisons: W vs N, W vs P, N vs P |

### Scientific Applications

* **Allele bin validation**: significant W < P difference with N intermediate confirms that the sequence-based bin structure maps onto functional recognition specificity
* **Synonymy detection**: N crosses that consistently fail (seed yield not significantly different from W) indicate that the allele bins involved are synonymous and could be merged
* **SI breakdown documentation**: W crosses yielding seeds identify individuals where the polyploid competitive interaction mechanism has enabled self-fertilisation, providing direct empirical evidence for Stage 5 of the Compatibility Collapse Cascade
* **Proposal preliminary data**: even a small initial crossing experiment (n = 20–30 crosses) covering all three categories provides sufficient preliminary evidence to justify the full Objective 2 crossing design in the NSF proposal

...
16. [TP1 Tipping Point Analysis](#TP1) – **Health of the SI system**: diagnostic scatter plot asking (x) how many different alleles a population has retained and (y) how evenly those alleles are distributed across individuals; with CRITICAL zone shading and TP1 status flags.
...
**Two-step analysis:** [Step 21](#alleleclusters) identifies hypervariable positions in the S-domain, detects phylogenetic classes by UPGMA clustering, and generates the prioritised crossing plan with four categories. [Step 22](#crossresults) reads completed cross records and tests whether the W / N / P_within / P_cross category predicts seed yield, providing the statistical confirmation needed to validate or refine the sequence-based allele definitions.
...
| `CROSS_TSV` | None | Set to cross results filename to activate Part 2 (Step 22) |
...
> Requires `SRK_functional_allele_groups.tsv` and `SRK_AAAA_cross_design_HV.tsv` from [Step 21](#alleleclusters), plus a completed cross results file. Run after crossing experiments have been conducted and seed yields recorded.

### Overview

This step reads completed cross records — a file with one row per cross, containing the mother and father plant IDs and a seed yield column — and assigns each cross to a W, N, P_within, or P_cross category using the HV-based class structure from Step 21. It then tests whether category predicts seed yield using Kruskal-Wallis and pairwise Mann-Whitney U tests, and produces a figure showing seed yield distributions and success rates per category.
...
| File | Description |
|------|-------------|
| `SRK_functional_allele_groups.tsv` | Allele class assignments from Step 21 |
| `SRK_AAAA_cross_design_HV.tsv` | Pairwise cross design with W / N / P_within / P_cross categories from Step 21 |

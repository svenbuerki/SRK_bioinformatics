...
11. [SRK S-Allele Genotyping Pipeline](#SRKfunctionalprotgenotype) – Assignment of distance-defined S-allele bins to individuals, producing long-format allele tables and binary genotype matrices for population genetic analysis.
...
13. [Allele Accumulation Curves](#alleleaccum) – Rarefaction-based analysis of SRK allele discovery across individuals to evaluate patterns consistent with negative frequency-dependent selection versus genetic drift. Includes estimation of total species allele richness using Michaelis-Menten asymptote fitting, Chao1, and iNEXT estimators. Also estimates how many additional individuals would need to be sampled to discover one more allele or to reach 80%, 90%, 95%, and 99% of the MM-estimated asymptote.
14. [Allele Frequency Analysis](#allelefrqanalysis) – Species- and population-level χ² tests of allele frequency distributions to assess deviations from equal-frequency expectations under negative frequency-dependent selection. The estimated species allele richness (from step 13) is used as the NFDS optimum baseline.
...
SRK_individual_allele_genotypes.tsv (wide binary matrix: individuals × alleles)
```
...
Multiple sequences from the same individual may map to the same protein, and multiple proteins may belong to the same allele bin. Both levels of redundancy are collapsed: the final allele set per individual contains only unique alleles.

#### Output generation

A long-format table (one row per individual–protein–allele triple) and a wide binary presence/absence matrix (individuals × alleles) are written. The long table retains protein-level information so the protein → allele mapping can be traced if needed.

### Output Files

| File | Format | Description |
|---|---|---|
| `SRK_individual_allele_table.tsv` | Long (Individual, Protein, Allele) | One row per unique individual–protein–allele combination |
| `SRK_individual_allele_genotypes.tsv` | Wide binary matrix | Rows = individuals, columns = allele bins |

**Long format example:**
```
Individual            Protein          Allele
Library001_barcode01  SRK_protein_001  Allele_021
Library001_barcode01  SRK_protein_045  Allele_009
Library001_barcode02  SRK_protein_001  Allele_021
```

**Wide format example:**
```
Individual            Allele_001  Allele_009  Allele_021
Library001_barcode01  0           1           1
Library001_barcode02  0           0           1
```
...
---------
  ...
  Per-individual allele set
        +
  SRK_individual_allele_table.tsv     long format  (Individual, Protein, Allele)
  SRK_individual_allele_genotypes.tsv wide matrix  (individuals x alleles, binary)
...
with open(WIDE_OUT, "w", newline="") as out:
    ...
    for ind in sorted(individual_alleles):
        aset = set(individual_alleles[ind])
        w.writerow([ind] + [1 if a in aset else 0 for a in all_alleles])

counts = [len(v) for v in individual_alleles.values()]
print(f"Individuals genotyped  : {len(individual_alleles)}")
print(f"S-alleles in matrix    : {len(all_alleles)}")
print(f"Max alleles/individual : {max(counts)}")
print(f"Mean alleles/individual: {sum(counts)/len(counts):.2f}")
```

...
Two-page PDF, four panels per page (Effective allele diversity, Heterozygosity, Allele richness, Sample size). **Page 1**: EO bars in connectivity order (parent BL by within-BL connectivity → EOs by ascending mean Drift_index), bars coloured by BL using the locked Set1 palette (legend in panel 1). **Page 2**: BL aggregate bars in connectivity order (BL5, BL1, BL4, BL3, BL2).
...
**BL-stratified design.** EOs are ordered by parent BL (within-BL connectivity, most connected at top) then by ascending mean Drift_index within BL; y-axis labels are coloured by parent BL (Set1 palette). The BL panel pools all BL-assigned individuals into 5 lineage rows, ordered by within-BL connectivity (BL5, BL1, BL4, BL3, BL2 — most connected at top). The TP2 AAAA threshold (30%) is marked on both panels.
...
**`SRK_GFS_reproductive_effort` (page 1, EO panel):** Each horizontal bar represents one EO with N ≥ 5. EOs are ordered by parent BL (within-BL connectivity, most connected at top) then by ascending mean Drift_index within BL; y-axis labels are coloured by parent BL using the Set1 palette. Segments are coloured by GFS class (red = AAAA through blue = ABCD), stacked left to right from lowest to highest GFS. The dashed vertical line marks the TP2 AAAA threshold (30%): bars where the red (AAAA) segment extends past this line indicate a TP2 breach. Annotations to the right show proportion supporting (%), count (n / N), and mean GFS.

**`SRK_GFS_reproductive_effort` (page 2, BL panel):** Same layout for the 5 BLs, ordered by within-BL connectivity (BL5, BL1, BL4, BL3, BL2 — most connected at top). All 325 BL-assigned individuals are aggregated into their respective BL rows; y-axis labels are coloured by BL.

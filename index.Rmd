...
**Two-step analysis:** [Step 22](#alleleclusters) identifies hypervariable positions in the S-domain, detects phylogenetic classes by UPGMA clustering, and generates the prioritised crossing plan with four categories. [Step 23](#crossresults) reads completed cross records and tests whether the W / N / P_within / P_cross category predicts seed yield, providing the statistical confirmation needed to validate or refine the sequence-based allele definitions.
...
This step implements a three-stage analysis to translate the sequence-based allele bins into a structured crossing experiment:

1. **Hypervariable (HV) position identification** — A moving-window scan of per-column variability across the S-domain alignment identifies the positions that actually discriminate alleles. Distances computed on these HV positions are biologically meaningful for recognition specificity, unlike full-domain distances which are dominated by conserved structural residues.

2. **Phylogenetic class detection** — UPGMA clustering of HV-only pairwise distances automatically detects the natural class structure. The largest gap in the merge-height sequence defines the cut point, splitting the 63 allele bins into Class I (62 alleles) and Class II (1 allele, Allele_061) — corresponding to the known phylogenetic lineages in *Brassica* SRK.

3. **Cross design** — All pairwise AAAA × AAAA combinations are categorised using HV distance and class membership into four categories (W, N, P_within, P_cross) and ranked to produce a practical crossing plan.
...
**Why focus on hypervariable positions?** Full S-domain p-distances are dominated by conserved structural positions shared across all alleles, producing a bimodal distance distribution in which most allele pairs collapse to near-zero distances and only the Class I / Class II split creates a large gap. The 75 HV positions (concentrated in seven regions of the S-domain: alignment positions 149–151, 187–199, 236–253, 271, 276–290, 360–363, and 365–385) correspond to the known HV1 and HV2 loops responsible for SCR ligand recognition. Distances computed on these positions alone resolve the fine-scale allele structure that is biologically relevant for SI specificity.

**Why auto-detect classes?** The largest gap in UPGMA merge heights objectively identifies the phylogenetically meaningful cut point without arbitrary parameter tuning. In the current dataset this threshold is 0.5045, correctly separating the well-known Class I and Class II *Brassica* SRK lineages. Class II alleles are guaranteed to be compatible with all Class I alleles — providing unambiguous P_cross controls at the maximum possible HV distance.

**Four-tier cross design:** The WITHIN_CLASS_THRESHOLD (default 0.04, approximately the median within-class HV distance) divides within-class pairs into three tiers. The N tier — small but non-zero HV differences — is the scientific core: an incompatible N cross means the two allele bins share recognition specificity (merge them); a compatible N cross confirms the small HV difference is functionally real (keep bins separate). In the current AAAA-only dataset, 6,273 N pairs are available for testing — the largest tier and the highest priority for the crossing experiment.
...
| `WINDOW_SIZE` | 20 | Moving-window size (aa) for variability scan |
| `PEAK_SD_FACTOR` | 1.0 | Positions above mean + PEAK_SD_FACTOR × SD of smoothed profile are classified as HV |
| `MIN_HV_POSITIONS` | 10 | Minimum HV positions required; script aborts if fewer detected |
| `WITHIN_CLASS_THRESHOLD` | 0.04 | HV distance boundary between N and P_within categories |
| `N_GROUPS` | None | Override auto-detected class count (None = use largest-gap auto-threshold) |
| `DISTANCE_THRESHOLD` | None | Override auto-detected UPGMA cut height (None = auto) |
| `CROSS_TSV` | None | Set to cross results filename to activate Part 2 (Step 23) |

#### Input Files

| File | Description |
|------|-------------|
| `SRK_protein_allele_representatives.fasta` | One representative sequence per allele bin (output of Step 10) |
| `SRK_individual_zygosity.tsv` | Per-individual tetraploid genotype pattern (output of Step 12) |
| `SRK_individual_metadata.tsv` | Individual metadata including EO and ingroup flag (output of Step 11) |

#### Output Files

| File | Content |
|------|---------|
| `SRK_variability_landscape.pdf` | Per-column mean pairwise distance and smoothed profile; shaded HV regions above detection threshold |
| `SRK_HV_allele_distances.tsv` | Pairwise HV-only p-distances for all 63 allele bin pairs |
| `SRK_allele_similarity_heatmap.pdf` | 63 × 63 HV similarity matrix ordered by UPGMA; colour scaled to within-class range |
...
| `SRK_cross_design_summary.pdf` | Three-panel figure: HV distance distribution by category; cross design schematic; N-cross interpretation decision tree |
| `SRK_HV_cluster_figure.pdf` | UPGMA dendrogram of allele bins coloured by class; AAAA individual count per bin |
| `SRK_synonymy_network_W.pdf` | Two-panel figure: HV-identical W-groups (left, individual alleles, node size ∝ individuals observed); isolated alleles with no HV-identical partner (right) |
| `SRK_synonymy_network_N.pdf` | Condensed super-node graph showing N-connectivity between W-groups; each node = one W-group or isolated allele; edges = N-bridges; node size ∝ individuals observed; edge width ∝ log(N-bridge count) |
| `SRK_synonymy_groups.csv` | Per-allele synonymy group membership table — see column descriptions below |

#### Key Output Statistics (current dataset: 272 individuals, 63 allele bins, 164 AAAA)

| Cross category | Definition | Pair count | Priority |
|----------------|-----------|:---:|---------|
| W | HV-identical (d = 0) | 5,766 | Negative control — confirms allele identity |
| N | 0 < d < 0.04, same class | 6,273 | **Synonymy test** — highest scientific priority |
| P_within | d ≥ 0.04, same class | 1,327 | Within-class positive control |
| P_cross | Different class | 0 (AAAA only) | Between-class control — requires AAAB parents |

Allele bin availability: 20 bins with ≥ 2 AAAA individuals (full design), 14 bins with 1 AAAA (partner only), 29 bins with no AAAA (require AAAB/AABB). Synonymy candidates: 1,891 within-class pairs; 561 testable with available AAAA individuals.

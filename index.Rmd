...
1. `SRK_TP1_compatibility_metrics.py` — computes per-EO and per-BL metrics, writes `Tables/SRK_EO_allele_richness.tsv`
2. `SRK_TP1_compatibility.R` — renders the four diagnostic panels
...
- **y = P_compat — the compatible-pair fraction** — *what fraction of random plant pairs are mating-compatible under tetraploid sporophytic SI?* A value of 0.40 means roughly 40 % of random pairs in the population can produce seed. Computed exactly from the inferred allele frequencies (formula below). Directly demographic — translates to expected fraction of compatible crosses.
...
Each panel ships with a `_blank.png` companion (empty quadrants, no points) for presentation overlays.

### Outputs

- `Tables/SRK_EO_allele_richness.tsv` — 5 BL rows + 6 EO rows × 17 metric columns (`level`, `group`, `BL`, `N`, `geno_mix`, `prop_AAAA`, `L_hat_from_AAAA`, `raw_copy_recovery`, `k_observed`, `k_rarefied30_mean/sd`, `shannon_H`, `evenness_J`, `chi2_uniform_p`, `P_compat_uniform_4x`, `P_compat_L0/L0.10/L0.25/L0.50`)
- Four PNG + four PDF panels (above)

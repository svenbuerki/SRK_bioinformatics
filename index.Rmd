...
18. [Individual Genotypic Fitness Score](#GFS) – Per-individual GFS quantifying heterozygous gamete proportion; EO-level Tipping Point 2 (TP2) status flags (CRITICAL / AT RISK / OK).
...
GFS is the operational metric for **Tipping Point 2 (TP2)**: the point at which the *distribution of alleles across individuals* within an EO has degraded to the extent that managed crossing cannot recover fitness without external allele introductions.

Two threshold conditions are evaluated for each EO:
...
An EO is flagged **CRITICAL** when both thresholds are breached simultaneously.

...
22. [Hypervariable-Position Allele Hypothesis Testing and Crossing Design](#alleleclusters) – Moving-window scan to identify 75 hypervariable positions in the S-domain; UPGMA auto-threshold clustering detects Class I / Class II phylogenetic split; generates a prioritised crossing plan for 164 AAAA individuals with four categories: W (HV-identical), N (synonymy test), P_within (within-class positive control), P_cross (between-class positive control).
23. [Cross Result Analysis](#crossresults) – Statistical comparison of seed yield across W / N / P_within / P_cross cross categories to test whether HV-based allele bin boundaries predict functional recognition specificity.
...
**`SRK_GFS_AAAA_allele_composition` (page 1, EO panel):** Companion figure unpacking the AAAA bar from the EO reproductive effort figure. Each bar shows how many AAAA individuals in that EO carry each allele. Pan-EO W-group 1 alleles (Allele_050, Allele_057; orange and blue) are highlighted; all remaining alleles are shown as a single grey "Other alleles" segment. Annotations show total AAAA count and percentage carrying a W-group 1 allele.

**`SRK_GFS_AAAA_allele_composition` (page 2, BL panel):** Same composition shown across the 5 BLs. Allele_050 and Allele_057 are pan-BL (5/5 BLs) — confirms shared W-group 1 fixation despite independent bottlenecks. BL2 reaches 69% W-group 1 saturation; BL5 is the lowest at 24%.
...
* **Independent-bottleneck test**: pan-BL Allele_050/057 confirms that drift across all five lineages has settled on the same W-group 1 specificity; the only EO without W-group 1 in its AAAA pool (EO18) is also the smallest, suggesting sample size rather than a divergent drift trajectory.
...
- **W — HV-identical** (same allele bin, HV distance = 0): pollen and pistil carry the same SRK allele at all hypervariable positions. Under a functional SI system, these crosses should fail — the pistil recognises the pollen as self and rejects it. These are negative controls. Any seed set observed in W crosses is direct evidence of SI breakdown, consistent with Stage 5 of the Compatibility Collapse Cascade.
...
- **P_within — within-class positive control** (HV distance ≥ 0.04, same phylogenetic class): pollen and pistil carry alleles that are substantially divergent within the same class. These crosses are expected to succeed and serve as within-class positive controls.

- **P_cross — between-class control** (different phylogenetic classes): pollen and pistil carry alleles from Class I and Class II respectively. These crosses are guaranteed to succeed — between-class compatibility is maximally divergent. They anchor the upper end of the seed yield scale.
...
| `WITHIN_CLASS_THRESHOLD` | 0.04 | HV distance boundary between N and P_within |
...
| `SRK_AAAA_cross_design_HV.tsv` | All AAAA × AAAA pairs ranked by category (W, N, P_within, P_cross) with HV distances and expected outcomes |
| `SRK_cross_design_summary.pdf` | Three-panel figure: HV distance distribution; cross design schematic; N-cross interpretation decision tree |
| `SRK_HV_cluster_figure.pdf` | UPGMA dendrogram of allele bins coloured by class; AAAA individual count per bin |
| `SRK_synonymy_network_W.pdf` | HV-identical W-groups (individual alleles, node size ∝ individuals observed); isolated alleles with no HV-identical partner |
| `SRK_synonymy_network_N.pdf` | Condensed super-node graph showing N-connectivity between W-groups |
...
|----------------|-----------|:---:|---------|
| W | HV-identical (d = 0) | 5 663 | Negative control — confirms allele identity |
| N | 0 < d < 0.04, same class | 6 365 | **Synonymy test** — highest scientific priority |
| P_within | d ≥ 0.04, same class | 1 338 | Within-class positive control |
| P_cross | Different class | 0 (AAAA only) | Between-class control — requires AAAB parents |

Allele bin availability: 20 bins with ≥ 2 AAAA individuals (full design), 14 bins with 1 AAAA (partner only), 29 bins with no AAAA (require AAAB/AABB). Synonymy candidates: 1 891 within-class pairs; 561 testable with available AAAA individuals. Synonymy network collapses 63 alleles into **9 W-groups + 23 isolated singletons = 32 effective bins** (40 of 63 alleles fall into W-groups, 23 are HV-unique).
...
| `Category` | **W**, **N**, **P_within**, or **P_cross** — key column for cross prioritisation |
...
|--------|-------------|
| `Allele` | Allele bin name |
| `Synonymy_group` | `W-group 1` through `W-group 9` for HV-identical clusters; `Isolated` for alleles with no HV-identical partner |
...
* **Cross prioritisation**: N pairs are the highest scientific priority — incompatibility confirms synonymy (merge bins); compatibility confirms the HV difference changes recognition specificity (retain bins)
* **Allele bin refinement**: compatible N crosses → bin boundary is real; incompatible N crosses → merge bins into one functional allele
* **Class validation**: P_cross crosses (requiring AAAB parents with Allele_061) provide maximum-distance compatible controls anchoring the upper end of the seed yield scale
* **Conservation breeding**: W crosses yielding seeds are direct evidence of SI breakdown — Stage 5 Compatibility Collapse Cascade events — and identify individuals for immediate priority in inter-EO allele rescue crosses
* **Extending the design**: adding AAAB parents unlocks the 29 allele bins currently without AAAA representatives and provides the P_cross controls absent from the AAAA-only dataset
...
This step reads completed cross records — a file with one row per cross, containing the mother and father plant IDs and a seed yield column — and assigns each cross to a W, N, P_within, or P_cross category using the HV-based class structure from Step 22. It then tests whether category predicts seed yield using Kruskal-Wallis and pairwise Mann-Whitney U tests, and produces a figure showing seed yield distributions and success rates per category.
...
The statistical test is a direct evaluation of the allele bin definitions using HV-distance-based categories. The expected ordering is W < N ≤ P_within < P_cross for seed yield: W crosses are rejected by SI (HV-identical alleles); N crosses test whether small HV differences change recognition specificity; P_within crosses are expected compatible within the same phylogenetic class; P_cross crosses between classes are maximally compatible and set the upper yield reference. A significant Kruskal-Wallis result with this ordering validates the HV-based bin structure. N crosses that fail (yield ≈ W) indicate synonymous allele bins that should be merged; N crosses that succeed confirm the small HV difference is functionally real. W crosses yielding seeds are direct evidence of SI breakdown.
...
| `SRK_AAAA_cross_design_HV.tsv` | Pairwise cross design with W / N / P_within / P_cross categories from Step 22 |
...
| File | Content |
|------|---------|
| `SRK_cross_result_analysis.pdf` | Box + strip plot of seed yield by category (W / N / P_within / P_cross); bar chart of cross success rate per category |
...
| Mann-Whitney U (pairwise) | Post-hoc comparisons: W vs N, W vs P_within, W vs P_cross, N vs P_within, N vs P_cross |

### Scientific Applications

* **Allele bin validation**: significant W < P_within difference with N intermediate confirms that the HV-based bin structure maps onto functional recognition specificity
* **Synonymy detection**: N crosses that fail (yield not significantly different from W) indicate synonymous allele bins — merge them; N crosses that succeed confirm the bin boundary is real
* **SI breakdown documentation**: W crosses yielding seeds identify individuals where the polyploid competitive interaction mechanism has enabled self-fertilisation — Stage 5 Compatibility Collapse Cascade events
* **Proposal preliminary data**: even a small initial crossing experiment (n = 20–30 crosses) covering W, N, and P_within categories provides sufficient preliminary evidence to justify the full Objective 2 crossing design in the NSF proposal

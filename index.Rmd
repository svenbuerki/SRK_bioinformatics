...
22. [Hypervariable-Position Allele Hypothesis Testing and Crossing Design](#alleleclusters) – Cross-Brassicaceae sliding-window Shannon-entropy scan identifies 73 canonical HV columns (validated by 10 000-permutation test against Brassica + Arabidopsis HV regions and structural overlap with Ma 2016 SCR9-contact residues); UPGMA auto-threshold clustering detects the Class I / Class II phylogenetic split; generates a sequence-based cross design for AAAA × AAAA pairs with four categories (Incompatible, Synonymy_test, Compatible_within, Compatible_cross). Optional diagnostics 22c (Synonymy group collapse test) and 22d (per-BL entropy decomposition) test alternative mechanisms underlying LEPA's low entropy. **Step 22e** ([Hypothesis-Testing Cross Plan](#crossplan)) translates the sequence-based categories into a phased experimental protocol with explicit AAAA / AAAB / AABB genotype constraints (H0 SI validation, H1a within-Class baseline, H1b between-Class baseline, H2 synonymy bin boundaries, H3 hidden bins via heterozygous donors + paired controls).
...
Input:
- MSA file: all_Library006_Phased_haplotypes_filtered_min3250_max4000_aligned.fasta
...
- Min exon length: 0
Output:
	- `all_Library003_Phased_haplotypes_filtered_min3250_max4000_aligned_exons.fasta`
...
- Input: 
	- MSA fasta file (exon): `all_Library006_Phased_haplotypes_filtered_min3250_max4000_aligned_exons.fasta`
	- Canonical reference ID (exact FASTA header): `SRK_BEA_hybrid_bp_hap1_p_ctg_fa|amp_1|h1tg000019l|1700582|3407`
- Output: `all_Library003_Phased_haplotypes_filtered_min3250_max4000_aligned_exons_backfilled.fasta`
...
- Input:
	- Exon aligned fasta: all_Library006_Phased_haplotypes_filtered_min3250_max4000_aligned_exons_backfilled.fasta
	- Position for translation (here, 1st)
- Output:
	- 4 files, but this one if important: `all_Library003_Phased_haplotypes_filtered_min3250_max4000_aligned_exons_backfilled_frame1_AA_filtered_aligned.fasta`
...
- **Input:** `*_exons_backfilled.fasta` (e.g., `all_Library003_Phased_haplotypes_filtered_min3250_max4000_aligned_exons_backfilled.fasta`)
    - frame = 1
    - filter = 5
    - max_alleles = 4
    - Enhanced = n
...
| `N_ALLELES` | `63` | **Option A** — fix the number of alleles directly (overrides `DIST_THRESHOLD` when not `None`). Use when you can identify the elbow on the sensitivity curve but cannot read the exact distance. For the current dataset (Libraries 001–009, 308 individuals, 302 functional proteins), the sensitivity curve showed a plateau between 50–100 alleles centred at 63 (implied threshold ≈ 0.005). |
...
> As of 2026-05-07 the script reads `SRK_individual_BL_assignments.tsv` and applies a **split-filter design**: the species-level curve uses ALL 272 ingroup individuals (so the species pool baseline is inclusive), while EO and BL curves use only the 262 BL-assigned individuals. The species pool stays at 54 observed / MM=69 / Chao1=73; without the split it would shrink to 50 / MM=63.
...
* `figures/SRK_allele_accumulation_BL_combined.png` — **NEW**. The 5 BL aggregate curves on shared axes; the headline visual test of "are independent bottleneck lineages approaching saturation independently?" Headline finding: BL4 climbs to 31 alleles toward MM=47 while BL2 plateaus at 8 alleles (MM=10).
...
**33 of 54 alleles (61%) are private to a single BL**; only 2 alleles (Allele_050 and Allele_057) are shared across all 5 BLs. The BL4 ↔ BL5 pair shares 12 alleles (highest off-diagonal in the heatmap), hinting at limited historical contact between these two lineages.
...
**`SRK_GFS_AAAA_allele_composition` (page 1, EO panel):** Companion figure unpacking the AAAA bar from the EO reproductive effort figure. Each bar shows how many AAAA individuals in that EO carry each allele. Pan-EO Synonymy group 1 alleles (Allele_050, Allele_057; orange and blue) are highlighted; all remaining alleles are shown as a single grey "Other alleles" segment. Annotations show total AAAA count and percentage carrying a Synonymy group 1 allele.

**`SRK_GFS_AAAA_allele_composition` (page 2, BL panel):** Same composition shown across the 5 BLs. Allele_050 and Allele_057 are pan-BL (5/5 BLs) — confirms shared Synonymy group 1 fixation despite independent bottlenecks. BL2 reaches 69% Synonymy group 1 saturation; BL5 is the lowest at 24%.
...
**Why this validation matters.** In the current dataset, all three pairwise HV-overlap permutation tests are highly significant (LEPA↔Brassica obs = 17 cols, p = 0.0005; LEPA↔Arabidopsis 22 cols, p = 0.0024; Brassica↔Arabidopsis 43 cols, p < 0.0001). All 11 of the 12 mappable Ma 2016 SCR9-contact residues fall within LEPA HV regions — independent structural confirmation that the entropy scan is detecting the SI-specificity surface. LEPA additionally has a unique HV peak at columns 358–388 that is absent from Brassica/Arabidopsis — a candidate Lepidium-specific specificity site flagged for future investigation.

**Phylogenetic class detection** — UPGMA clustering of the resulting 63 × 63 HV-only pairwise distances automatically detects the natural class structure: the largest gap in the merge-height sequence defines the cut point, splitting the 63 allele bins into Class I (62 alleles) and Class II (1 allele, Allele_061) — corresponding to the known phylogenetic lineages in *Brassica* SRK. Class II alleles are guaranteed to be compatible with all Class I alleles, providing unambiguous Compatible_cross controls at the maximum possible HV distance.
...
* Helper scripts (one-time setup): `srk_fetch_reference_alleles.py`, `srk_brassica_hv_mapping.py`
* Environment: Python 3 with numpy, pandas, matplotlib, scipy, biopython, plus `mafft` on PATH
* Execution (run from project root):

```bash
# One-time setup (fetch references + add Brassica reference for Ma 2016 markers)
...
# Step 22a — variability landscape (re-run when LEPA representatives change)
...
| `SRK_LEPA_HV_positions.tsv` | **73 canonical LEPA HV columns** (consumed by Step 22b) |
...
| File | Content |
|------|---------|
| `SRK_HV_allele_distances.tsv` | Pairwise HV-only p-distances for all 63 allele bin pairs (computed on the 73 canonical HV cols) |
...
#### Key Output Statistics (current dataset: 272 individuals, 63 allele bins, 164 AAAA, 73 canonical HV cols)

| Cross category | Definition | Pair count | Priority |
|----------------|-----------|:---:|---------|
| Incompatible | HV-identical (d = 0) | 5 663 | Negative control — confirms allele identity |
| Synonymy_test | 0 < d < 0.04, same class | 6 365 | **Synonymy test** — highest scientific priority |
| Compatible_within | d ≥ 0.04, same class | 1 338 | Within-class positive control |
| Compatible_cross | Different class | 0 (AAAA only) | Between-class control — requires AAAB parents |

Allele bin availability: 20 bins with ≥ 2 AAAA individuals (full design), 14 bins with 1 AAAA (partner only), 29 bins with no AAAA (require AAAB/AABB). Synonymy candidates: 1 891 within-class pairs; 561 testable with available AAAA individuals. Synonymy network collapses 63 alleles into **9 synonymy groups + 23 isolated singletons = 32 effective bins** (40 of 63 alleles fall into synonymy groups, 23 are HV-unique).
...
* **Class validation**: Compatible_cross crosses (requiring AAAB parents with Allele_061) provide maximum-distance compatible controls anchoring the upper end of the seed yield scale
...
|----------|---------------|-------------|---------------------|
| Hypervariable (HV) columns identifying the SI-recognition surface | `srk_variability_landscape.py` | `SRK_LEPA_HV_positions.tsv` | Shannon entropy > mean + 1×SD on a 20 aa smoothed profile, with min run of 3 cols. Validated cross-Brassicaceae by 10 000-permutation test (LEPA↔Brassica p = 0.0005, LEPA↔Arabidopsis p = 0.0024) and structural overlap with the 12 SCR9-contact residues from the Ma et al. 2016 *B. rapa* eSRK9–SCR9 crystal structure (PDB 5GYY) |
| Pairwise HV distance | `srk_allele_hypotheses.py` Part 2 | `SRK_HV_allele_distances.tsv` | p-distance computed only on the 73 canonical HV columns (not the full S-domain) |
...
1. **The 73 HV columns are the SI-recognition surface.** Justified by cross-Brassicaceae permutation tests (3 pairs all p ≤ 0.0024) and by structural overlap with the Ma 2016 *B. rapa* eSRK9–SCR9 crystal structure.
...
#### Key Output Statistics (current dataset: 272 individuals, 63 allele bins, 164 AAAA, 9 synonymy groups + 23 isolated)
...
**Sample-size constraints in the current dataset.** H1b is sample-limited to 1 cross because Allele_061 (the only Class II allele) has a single carrier in the dataset (Library008_barcode08, AABB Allele_050+Allele_061). H3 covers 20 of the 29 hidden bins; the remaining 9 lack AAAA mothers carrying a Compatible_within partner allele (the constraint required to keep the AA-pollen baseline interpretable). Adding additional Allele_061 carriers and additional AAAA mothers would directly relax both constraints. The carrier-detection logic reads `Allele_composition` strings authoritatively, so the cross plan automatically expands as new individuals are added — re-run Step 22a → 22b → 22e after new genotyping data lands.
...
- **H2 outcomes** decide which synonymy groups should be **merged** (0 seeds → same SI specificity) or **kept separate** (yield ≥ H1a baseline → distinct specificities). Most extreme case: every Synonymy_test cross is incompatible → 63 sequence bins collapse to **32 functional specificities** (9 synonymy groups + 23 isolated alleles).
